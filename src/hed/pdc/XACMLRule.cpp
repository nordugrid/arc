...
void XACMLRule::getItemlist(XMLNode& nd, OrList& items, const std::string& itemtype, const std::string& type_attr, const std::string& function_attr){
  XMLNode tnd;
  ...
  for(int i=0; i<nd.Size(); i++){
    ...
    for(int j=0;;j++){
      ...
      if(!(type.empty())&&(tnd.Size()==0)){
        ...
        logger.msg(Arc::ERROR, "Error definition in rule"); 
        return;
      }
