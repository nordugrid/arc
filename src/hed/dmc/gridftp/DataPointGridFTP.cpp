...
namespace Arc {
  ...
  (void *arg, globus_ftp_client_handle_t*,
   globus_object_t *error) {
    ...
    if(error == GLOBUS_SUCCESS) {
      ...
      logger.msg(INFO, "ftp_complete_callback: error: %s", tmp.c_str());
  ...
  (void *arg, globus_ftp_client_handle_t*,
   ...
   globus_off_t, globus_bool_t eof) {
    ...
    if(!res) {
      ...
      logger.msg(DEBUG, "Globus error: %s", res.str().c_str());
  ...
  bool DataPointGridFTP::start_reading(DataBufferPar& buf) {
    ...
    if(limit_length) {
      res = globus_ftp_client_partial_get
        (&ftp_handle, url.str().c_str(), &ftp_opattr,
  ...
  bool DataPointGridFTP::stop_reading() {
  ...
  (void *arg, globus_ftp_client_handle_t*,
   globus_object_t *error) {
    ...
    if(error != GLOBUS_SUCCESS) {
      ...
      it->failure_description = globus_object_to_string(error);
  ...
  bool DataPointGridFTP::start_writing(DataBufferPar& buf, DataCallback*) {
    ...
    if(limit_length) {
      res = globus_ftp_client_partial_put
        (&ftp_handle, url.str().c_str(), &ftp_opattr, GLOBUS_NULL,
  ...
  bool DataPointGridFTP::stop_writing() {
  ...
  bool DataPointGridFTP::list_files(std::list<FileInfo>& files, bool resolve) {
