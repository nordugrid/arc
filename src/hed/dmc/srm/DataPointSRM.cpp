...
namespace Arc {
  ...
  DataStatus DataPointSRM::Remove() {
    ...
    SRMClient * client = SRMClient::getInstance(url.fullstr());
    if(!client) return DataStatus::DeleteError;
    
    // take out options in srm url
    std::string canonic_url;
    if (!url.HTTPOption("SFN").empty()) canonic_url = url.Protocol()+"://"+url.Host()+"/"+url.HTTPOption("SFN");
    else canonic_url = url.Protocol()+"://"+url.Host()+"/"+url.Path();
    
    srm_request = new SRMClientRequest(canonic_url);

    if(!srm_request) { delete client; client=NULL; return DataStatus::DeleteError; }
    logger.msg(DEBUG, "remove_srm: deleting: %s", CurrentLocation().str());
    
    if(!client->remove(*srm_request)) { delete client; client = NULL; return DataStatus::DeleteError; }
  ...
  DataStatus DataPointSRM::StartReading(DataBuffer& buf) {
    ...
    SRMClient * client = SRMClient::getInstance(url.fullstr());
    if(!client) { reading = false; return DataStatus::ReadError; };
    ...
    srm_request = new SRMClientRequest(canonic_url);
    if(!srm_request) { delete client; client = NULL; return DataStatus::ReadError; }
    ...
    if(!client->getTURLs(*srm_request,turls)) { delete client; client = NULL; return DataStatus::ReadError; };
    client->disconnect();
    ...
    for(;;) {
      ...
      if(!r_handle) { turls.erase(i); continue; };
      if((*r_handle)->ProvidesMeta()) { delete r_handle; r_handle = NULL; turls.erase(i); continue; };
      break;
    }
  ...
  DataStatus DataPointSRM::StopReading() {
    ...
    if(!r_handle) return DataStatus::Success;
    ...
    r_handle = NULL; srm_request = NULL;
    return r;
  }
  ...
  DataStatus DataPointSRM::StartWriting(DataBuffer& buf,
    ...
    SRMClient * client = SRMClient::getInstance(url.fullstr());
    if(!client) { writing = false; return DataStatus::WriteError; };
    ...
    srm_request = new SRMClientRequest(canonic_url);
    if(!srm_request) { delete client; client = NULL; return DataStatus::WriteError; }
    ...
    std::list<std::string> turls;
    if(!client->putTURLs(*srm_request,turls)) { delete client; client = NULL; return DataStatus::WriteError; };
    client->disconnect();
    ...
    if(r_handle == NULL) {
      logger.msg(INFO, "SRM returned no useful Transfer URLs: %s", url);
      delete client; client=NULL; return DataStatus::WriteError;
    }
  ...
  DataStatus DataPointSRM::ListFiles(std::list<FileInfo>& files,
    ...
    srm_request = new SRMClientRequest(canonic_url);
    if(!srm_request) { delete client; client=NULL; return DataStatus::ListError; }

    logger.msg(DEBUG, "ListFiles: looking for metadata: %s", CurrentLocation().str());
    std::list<struct SRMFileMetaData> metadata;

    // get info from SRM
    if(!client->info(*srm_request,metadata)) { delete client; client=NULL; return DataStatus::ListError; }
    
    if(metadata.empty()) { delete client; client=NULL; return DataStatus::Success; }
    ...
    if(metadata.front().createdAtTime > 0) SetCreated(Time(metadata.front().createdAtTime));
