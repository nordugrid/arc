...
namespace Arc {
  ...
  void Job::Print(bool longlist) const {
	...
	std::cout << Arc::IString(" Other Messages: %s", OtherMessages)
		  << std::endl;
      if (!ExecutionCE.empty())
	std::cout << Arc::IString(" ExecutionCE: %s", ExecutionCE)
		  << std::endl;
      if (!Queue.empty())
	...
	std::cout << Arc::IString(" Waiting Position: %d", WaitingPosition)
		  << std::endl;
      if (!StdIn.empty())
	...
	std::cout << Arc::IString(" Stderr: %s", StdErr) << std::endl;
      if (!LogDir.empty())
	std::cout << Arc::IString(" Grid Manager Log Directory: %s", LogDir)
		  << std::endl;
      if (SubmissionTime != -1)
	std::cout << Arc::IString(" Submitted: %s",
				  (std::string)SubmissionTime) << std::endl;
      if (EndTime != -1)
	std::cout << Arc::IString(" End Time: %s", (std::string)EndTime)
		  << std::endl;
      if (!SubmissionHost.empty())
	std::cout << Arc::IString(" Submitted from: %s", SubmissionHost)
		  << std::endl;
      if (!SubmissionClientName.empty())
	std::cout << Arc::IString(" Submitting client: %s",
				  SubmissionClientName) << std::endl;
      if (RequestedTotalCPUTime != -1)
	std::cout << Arc::IString(" Requested CPU Time: %s",
				  (std::string)RequestedTotalCPUTime)
		  << std::endl;
      if (UsedTotalCPUTime != -1)
	std::cout << Arc::IString(" Used CPU Time: %s",
				  (std::string)UsedTotalCPUTime) << std::endl;
      if (UsedWallTime != -1)
	std::cout << Arc::IString(" Used Wall Time: %s",
				  (std::string)UsedWallTime) << std::endl;
      if (UsedMainMemory != -1)
	std::cout << Arc::IString(" Used Memory: %d", UsedMainMemory)
		  << std::endl;
	...
	std::cout << Arc::IString(" Proxy valid until: %s",
				  (std::string)ProxyExpirationTime)
		  << std::endl;
      if (CreationTime != -1)
	std::cout << Arc::IString(" Entry valid from: %s",
				  (std::string)CreationTime) << std::endl;
      if (Validity != -1)
	std::cout << Arc::IString(" Entry valid for: %s",
