...
namespace Arc {
  ...
  void ExecutionTarget::Print(bool longlist) const{
	...
	std::cout << IString(" Longitude: %f", Longitude) << std::endl;
      ...
      if (Cluster)
	std::cout << IString(" URL: %s", Cluster.str()) << std::endl;
      if (!Interface.empty()){
	std::cout << IString(" Interface Name: %s", Interface) << std::endl;
	if (!InterfaceExtension.empty())
	  std::cout << IString(" Interface Extension: %s", InterfaceExtension) << std::endl;
      }
	...
	std::cout << IString(" Default Storage Service: %s", DefaultStorageService.str()) << std::endl;
      if (Preemption)
