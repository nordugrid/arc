...
static unsigned long long int cache_clean(const std::string& cache_path,const std::string& cache_data_path,uid_t cache_uid,gid_t cache_gid,unsigned long long int size,int h);
...
/*
  -1 - error,
   ...
   1 - not found
*/
static int find_record(int h,const std::string& fname,off_t &record_start,unsigned int &record_length,bool match_full = false) {
  char buf[1024];
...
// ------------------------------------------------------------------
static int cache_history_add_record(const std::string& fname,const std::string& name);
static int cache_history_rem_record(const std::string& fname,const std::string& name);

static int cache_history_add_new(const std::string& cache_path,const std::string& name) {
...
static int cache_history_add_old(const std::string& cache_path,const std::string& name) {
  std::string fname(cache_path);
  fname+="/old";
  int n=cache_history_add_record(fname,name);
  if(n!=0) return n;
...
static int cache_history_add_record(int h,const std::string& name) {
  ...
  if(write_all(h,name.c_str(),name_l) != name_l) return -1;
  return 0;
}
...
static int cache_open_list(const std::string& cache_path,uid_t uid,gid_t gid) {
  std::string fname(cache_path);
  fname+="/list";
...
/*
  ...
  returns: 0 - found, 1 - not found, -1 - error
*/
static int cache_search_list(int h,const std::string& url,std::string &fname) {
  if(h==-1) return -1;
  lseek(h,0,SEEK_SET);
...
static int cache_open_info(const std::string& cache_path,const std::string& fname) {
...
*/
  static int cache_add_list(int h,const std::string& url,const std::string& cache_path,const std::string& cache_data_path,uid_t cache_uid,gid_t cache_gid,const std::string &id,std::string &fname) {
#define OUT_OF_SPACE_CHECK                                                    \
  ...
  for(;i<INT_MAX;i++) {
    ...
    int nh=open(name.c_str(),O_CREAT | O_EXCL | O_RDWR,S_IRUSR | S_IWUSR);
    if(nh == -1) {
      OUT_OF_SPACE_CHECK;
      continue;
    };
    ...
    nh=::open(name_info.c_str(),O_CREAT | O_EXCL | O_RDWR,S_IRUSR | S_IWUSR);
    if(nh == -1) { 
      OUT_OF_SPACE_CHECK;
      ::remove(name.c_str()); continue;
    };
    if(write_all(nh,"c\n",2) == -1) {
      OUT_OF_SPACE_CHECK;
      close(nh); remove(name.c_str()); remove(name_info.c_str()); continue;
    };
    close(nh);
    name_claim = cache_path + '/' + name_ + ".claim";
    nh=open(name_claim.c_str(),O_CREAT | O_EXCL | O_RDWR,S_IRUSR | S_IWUSR);
    if(nh == -1) {
      OUT_OF_SPACE_CHECK;
      remove(name.c_str()); remove(name_info.c_str()); continue;
    };
    ...
    close(nh);
    if(cache_uid != 0) {
      (void)chown(name.c_str(),cache_uid,cache_gid);
      (void)chown(name_info.c_str(),cache_uid,cache_gid);
      (void)chown(name_claim.c_str(),cache_uid,cache_gid);
    }
    else {
  ...
  if(find_empty_space(h,record_l) == -1) {
    remove(name.c_str());remove(name_info.c_str());
    return -1;
  };
...
/*
  ...
  non-claimed files 
*/
static int cache_remove_list(int h,const std::string& fname,const std::string& cache_path,const std::string& cache_data_path,uid_t cache_uid,gid_t cache_gid) {
  if(h==-1) return -1;
...
static int cache_invalidate_list(int h,const std::string& cache_path,const std::string& cache_data_path,uid_t cache_uid,gid_t cache_gid,const std::string& fname) {
  if(h==-1) return -1;
  lseek(h,0,SEEK_SET);
  /* look through file for given name */
  off_t record_start = 0;
...
/* replace id (url) */ 
static int cache_replace_list(int h,const std::string& fname,const std::string& url) {
  if(h==-1) return -1;
  lseek(h,0,SEEK_SET);
  /* look through file for given name */
  off_t record_start = 0;
...
*/
static int cache_release_file(const std::string& cache_path,const std::string &id,const std::string& fname,bool once = false) {
...
int cache_is_claimed_file(const std::string& cache_path,const std::string& fname) {
  std::string name = cache_path + '/' + fname + ".claim";
  struct stat st;
  if(stat(name.c_str(),&st) != 0) return 1; /* probably missing == free */
  if(st.st_size == 0) return 1;
  return 0;
}

static int cache_file_info(const std::string& cache_path,const std::string& cache_data_path,const std::string& fname,bool &claimed,unsigned long long int &size,time_t &accessed) {
  struct stat st;
  std::string name = cache_path + '/' + fname + ".claim";
  if(stat(name.c_str(),&st) != 0) { claimed=false; }
  else {
...
*/
static int cache_claim_file(const std::string& cache_path,const std::string &id,const std::string& fname) {
  std::string name = cache_path + '/' + fname + ".claim";
  int h = open(name.c_str(),O_RDWR);
  if(h == -1) { return -1; };
  if(lock_file(h) != 0) { close(h); return -1; };
...
static char cache_read_info_nonblock(const std::string& cache_path,const std::string& fname) {
  std::string name = cache_path + '/' + fname + ".info";
  ...
  for(;;) {
    switch(cache_read_list(ch,url,fname)) {
      case 0: {  /* have url and name */
        if(cache_release_file(cache_path,id,fname) == -1) { fail=true;}
        else {
          if(remove) {
            char state=cache_read_info_nonblock(cache_path,fname);
            if(((state=='f') || (state=='c')) &&
               (cache_is_claimed_file(cache_path,fname)==1)) {
              if(cache_remove_list(ch,fname,cache_path,cache_data_path,cache_uid,cache_gid) != 0) {
                fail=true; 
              };
            };
          };
...
int cache_download_url_start(const std::string& cache_path,const std::string& cache_data_path,uid_t cache_uid,gid_t cache_gid,const std::string& url,const std::string &id,cache_download_handler &handler) {
  ...
  return cache_download_file_start(cache_path,cache_data_path,cache_uid,cache_gid,fname,id,handler);
}
...
class cache_file_p {
 public:
  ...
  time_t accessed;
  bool valid;
  cache_file_p(const std::string& name_,unsigned long long int size_,time_t accessed_,bool valid_):name(name_),size(size_),accessed(accessed_),valid(valid_) {  };
...
static unsigned long long int cache_clean(const std::string& cache_path,const std::string& cache_data_path,uid_t cache_uid,gid_t cache_gid,unsigned long long int size,int h) {
  ...
  for(std::list<cache_file_p>::iterator i=files.begin();i!=files.end();++i) {
    ...
    if(cache_remove_list(ch,i->name,cache_path,cache_data_path,cache_uid,cache_gid) == 0) { 
      total_size+=i->size;
    };
