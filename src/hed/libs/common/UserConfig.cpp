...
namespace Arc {
  ...
  void UserConfig::ApplyToConfig(BaseConfig& ccfg) const {
    ...
    switch(authType) {
      case AuthTypeNone:
         ccfg.cert_for_auth = false;
         ccfg.otoken_for_auth = false;
         break;
      case AuthTypeCert:
         ccfg.cert_for_auth = true;
         ccfg.otoken_for_auth = false;
         break;
      case AuthTypeToken:
         ccfg.cert_for_auth = false;
         ccfg.otoken_for_auth = true;
         break;
  ...
  bool UserConfig::JobListType(const std::string& type) {
    if (type != "XML" && type != "SQLITE") {
      logger.msg(WARNING, "Unsupported job list type '%s', using 'SQLITE'. Supported types are: SQLITE, XML.", type);
      joblisttype = "SQLITE";
      return true;
    }
