...
namespace Arc {
  ...
  template <typename T> class CountedPointer {
   private:
    template <typename P> class Base {
     private:
      Base(Base<P>&) { };
     public:
      int cnt;
      P* ptr;
      Base(P* p):cnt(0),ptr(p) { add(); };
      ~Base(void) { if(ptr) delete ptr; };
      Base<P> add(void) { ++cnt; return this; };
      bool rem(void) {
        if(--cnt == 0) {
          delete this;
          return true;
    ...
    Base<T>* object;
   public:
    CountedPointer(T* p):object(new Base<T>(p)) { };
    CountedPointer(CountedPointer<T>& p):object(p.object->add()) { };
    ~CountedPointer(void) { object->rem(); };
    CountedPointer& operator=(T* p) {
      if(p != object->ptr) {
        object->rem(); object=new Base<T>(p);
      };
      return *this;
    };
    CountedPointer& operator=(CountedPointer& p) {
      if(p.object->ptr != object->ptr) {
        object->rem(); object=p.object->add();
      };
      return *this;
    };
    /// For refering wrapped object
    T& operator*(void) const { return *(object->ptr); };
    /// For refering wrapped object
    T* operator->(void) const { return (object->ptr); };
    /// Returns false if pointer is NULL and true otherwise.
    operator bool(void) const { return ((object->ptr)!=NULL); };
    /// Returns true if pointer is NULL and false otherwise.
    bool operator!(void) const { return ((object->ptr)==NULL); };
    /// Cast to original pointer
    operator T*(void) const { return (object->ptr); };
  };
