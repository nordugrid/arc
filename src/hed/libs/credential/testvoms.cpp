...
int main(void) {
  ...
  * In real scenario the Attribute Certificate with string format should be received from the other end,
  ...
  */
  ...
  //Arc::Credential request(t, Arc::Period(12*3600), keybits, "rfc", "inheritAll", "", proxydepth);
  ...
  proxy.SetProxyPolicy("rfc", "inheritAll", "", proxydepth);
...
//  vomscert_trust_dn.push_back("NEXT CHAIN");
  ...
  Arc::parseVOMSAC(proxy2, ca_cert_dir, ca_cert_file, false, "", trust_dn, attributes);
