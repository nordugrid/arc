...
bool PrivateKeyInfoCodec::ReadIntegerWithExpectedSize(uint8** pos,
  ...
  if (out->size() == expected_size + 1) {
    if(out->front() != 0x00) return false;;
    pad++;
    index++;
  } else {
...
void PrivateKeyInfoCodec::PrependLength(size_t size, std::list<uint8>* data) {
  ...
  if (size < 0x80) {
    ...
    if(!(num_bytes <= 4)) std::cerr<<"num of bytes if bigger than 4"<<std::endl;
...
bool PrivateKeyInfoCodec::ReadLength(uint8** pos, uint8* end, uint32* result) {
  ...
  if (!(**pos & 0x80)) {
    ...
    if(!(length_of_length <= 4)) return false;
    (*pos)++;
    if(!(*pos + length_of_length < end)) return false;
...
bool PrivateKeyInfoCodec::ReadAlgorithmIdentifier(uint8** pos, uint8* end) {
  if(!(*pos + sizeof(kRsaAlgorithmIdentifier) < end)) return false;
  if(!(memcmp(*pos, kRsaAlgorithmIdentifier,
                     sizeof(kRsaAlgorithmIdentifier)) == 0)) return false;
