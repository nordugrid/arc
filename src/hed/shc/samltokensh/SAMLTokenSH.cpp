...
SAMLTokenSH::SAMLTokenSH(Config *cfg,ChainContext*,Arc::PluginArgument* parg):SecHandler(cfg,parg),
                                       system_ca_dir_(false), system_ca_file_(false), grid_ca_(true), valid_(false) {
  ...
  if(process_type == "generate") { 
    ...
    if(!(system_ca_dir_ || system_ca_file_) && ((ca_file_.empty() && ca_dir_.empty() && grid_ca_) || !grid_ca_)) {
  ...
  if(!cert_file_.empty()) {
    Arc::Credential cred(cert_file_, key_file_, ca_dir_, ca_file_, system_ca_dir_, system_ca_file_, grid_ca_);
...
*/
      if((!ca_file_.empty() || !ca_dir_.empty()) &&
          !st.Authenticate(grid_ca_?ca_file_:"", grid_ca_?ca_dir_:"", system_ca_dir_, system_ca_file_)) {
    ...
    try {
      if(!saml_assertion_) {
        ...
        cfg.SetSystemCA(system_ca_dir_, system_ca_file_);
        cfg.SetGridCA(grid_ca_);
        ...
        if(attr_resp_secnode.VerifyNode(resp_idname, grid_ca_?ca_file_:"", grid_ca_?ca_dir_:"", system_ca_dir_, system_ca_file_, true)) {
        ...
        if(assertion_secnode.VerifyNode(assertion_idname, grid_ca_?ca_file_:"", grid_ca_?ca_dir_:"", system_ca_dir_, system_ca_file_, true)) {
