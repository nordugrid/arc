...
namespace Arc {
  ...
  ServiceEndpointRetriever::ServiceEndpointRetriever(UserConfig userconfig,
                                                     ...
                                                     ServiceEndpointConsumer& consumer,
                                                     bool recursive,
                                                     std::list<std::string> capabilityFilter) : userconfig(userconfig){
    for (std::list<RegistryEndpoint>::iterator it = registries.begin(); it != registries.end(); it++) {
      ThreadArgSER* arg = CreateThreadArg(*it, consumer, capabilityFilter);
  ...
  void ServiceEndpointRetriever::QueryRegistry(void *arg_) {
    ThreadArgSER* arg = (ThreadArgSER*)arg_;
    
    ServiceEndpointRetrieverPluginLoader loader;
    ServiceEndpointRetrieverPlugin* plugin = loader.load("TEST");
    
    if (plugin) {
      logger.msg(Arc::DEBUG, "Querying " + arg->registry.Endpoint);
      arg->ser->setStatusOfRegistry(arg->registry, plugin->Query(arg->ser->userconfig, arg->registry, *arg->consumer));
    }
    delete arg;
  }  
