...
/** This two classed are MCCs implementing TLS functionality. Upon creation this 
  ...
  PayloadStreamInterface (actually PayloadTCPSocket), and the method return empty 
  payload in "outmsg", just as MCC_HTTP_Service does. The ssl object is created and 
  binded to socket object when constructing the PayloadTLSSocket in the process() method. 

  The process() method of MCC_TLS_Client is also passed payload impementing 
  PayloadStreamInterface and return empty payload. 

  So the MCC_TLS_Service and MCC_TLS_Client will only keep the information about SSL_CTX, 
  nothing else.  It is the PayloadTLSSocket some keeps some information about ssl session. 
  And the PayloadTLSSocket which implements the PayloadStreamInterface will be used by 
  PayloadHTTP.
*/
...
/** This class is MCC implementing TLS client.
  Unfortunately, the MCC_TLS_Client would be put behind MCC_TCP_Client, which looks 
  different with server side(MCC_TLS_Server is put between MCC_HTTP_Server and MCC_TCP_Server).
  the MCC_TLS_Client should get the socket fd and attache (I use "attache" :)) ssl to the fd, 
  and the socket fd is created in MCC_TCP_client and also used as s_.Put() in MCC_TCP_client 
  to flush tcp request, the ssl attachement should be done before s_.Put() call. So I just put 
  MCC_TLS_Client behind MCC_TCP_Client.
  Also there PayloadTLSStream that implement PayloadStreamInterface, which is specified for 
  TLS method, such like "SSL_read()" "SSL_write".
