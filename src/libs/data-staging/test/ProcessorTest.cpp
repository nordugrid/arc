...
void ProcessorTest::TestPreClean() {
  ...
  int h = Arc::FileOpen(destination, O_WRONLY | O_CREAT);
  CPPUNIT_ASSERT(h > 0);
...
void ProcessorTest::TestCacheCheck() {
  ...
  int fd = Arc::FileOpen(lock_file, O_WRONLY|O_CREAT);
  CPPUNIT_ASSERT(fd);
  ...
  if (valid_proxy) {
    ...
    fd = Arc::FileOpen(cache_file, O_WRONLY|O_CREAT);
    CPPUNIT_ASSERT(fd);
...
void ProcessorTest::TestCacheProcess() {
  ...
  conf_file += "/arc.conf";
  int fd = Arc::FileOpen(conf_file, O_WRONLY|O_CREAT);
  CPPUNIT_ASSERT(fd);
