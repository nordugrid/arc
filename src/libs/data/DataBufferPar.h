...
class DataBufferPar {
 ...
 public:
  ...
  operator bool(void) { return (bufs!=0); };
  /// Contructor
  ...
  /// \param size size of every buffer in bytes.
  /// \param blocks number of buffers.
  /// \param cksum object which will compute checksum. Should not be destroyed till DataBufferPar itself.
  ...
  /// \param wait if true and there are no free buffers, method will wait for one.
  ...
  unsigned long long int eof_position(void) const { return eof_pos; };
