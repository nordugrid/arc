...
void DataPointGridFTP::ftp_complete_callback(void* arg,globus_ftp_client_handle_t* handle,globus_object_t* error) {
...
void DataPointGridFTP::ftp_check_callback(void* arg,globus_ftp_client_handle_t* handle,globus_object_t* error,globus_byte_t* buffer,globus_size_t length,globus_off_t offset,globus_bool_t eof) {
  ...
  if(res != GLOBUS_SUCCESS) {
    logger.msg(INFO, "Registration of Globus FTP buffer failed - cancel check");
...
bool DataPointGridFTP::check(void) {
  ...
  res = globus_ftp_client_size(&ftp_handle,url.CanonicalURL().c_str(),&ftp_opattr,
  ...
  res = globus_ftp_client_modification_time(&ftp_handle,url.CanonicalURL().c_str(),
                     &ftp_opattr,&gl_modify_time,&ftp_complete_callback,this);
  if(res != GLOBUS_SUCCESS) {
    logger.msg(DEBUG, "check_ftp: globus_ftp_client_modification_time failed");
    logger.msg(INFO, "Globus error: %s", GlobusResult(res).str().c_str());
    // return false;
  } else if(!cond.timed_wait(mutex, x(300))) { /* 5 minutes timeout */
  ...
  if(is_secure) {
    res=globus_ftp_client_partial_get(&ftp_handle,url.CanonicalURL().c_str(),&ftp_opattr,
...
bool DataPointGridFTP::remove(void) {
  ...
  //char buf[32];
  globus_result_t res;
  res=globus_ftp_client_delete(&ftp_handle,url.CanonicalURL().c_str(),&ftp_opattr,
...
bool DataPointGridFTP::mkdir_ftp() {
  ...
  for(;;) {
    ...
    if(res != GLOBUS_SUCCESS) {
      logger.msg(INFO, "Globus error: %s", GlobusResult(res).str().c_str()); return false;
    };
...
bool DataPointGridFTP::start_reading(DataBufferPar &buf) {
  ...
  ftp_eof_flag=false;
  globus_ftp_client_handle_cache_url_state(&ftp_handle,url.CanonicalURL().c_str());
  globus_result_t res;
  if((!no_checks) && (!(meta_size_available()))) {
    logger.msg(DEBUG, "start_reading_ftp: size: url: %s", url.CanonicalURL().c_str());
    ...
    if(res != GLOBUS_SUCCESS) {
      ...
      logger.msg(INFO, "Globus error: %s", GlobusResult(res).str().c_str());
      // globus_ftp_client_handle_flush_url_state(&ftp_handle,url.CanonicalURL().c_str());
      ...
      /* timeout - have to cancel operation here */
      logger.msg(INFO, "Timeout waiting for FTP file size - cancel transfer");
      globus_ftp_client_abort(&ftp_handle);
  ...
  if((!no_checks) && (!(meta_created_available()))) {
    ...
    res = globus_ftp_client_modification_time(&ftp_handle,url.CanonicalURL().c_str(),
                     &ftp_opattr,&gl_modify_time,&ftp_complete_callback,this);
    if(res != GLOBUS_SUCCESS) {
      logger.msg(DEBUG, "start_reading_ftp: globus_ftp_client_modification_time failed");
      logger.msg(INFO, "Globus error: %s", GlobusResult(res).str().c_str());
      //buffer->error_read(true);
      // DataPointDirect::stop_reading();
      //return false;
    } else if(!cond.timed_wait(mutex, x(300))) { /* 5 minutes timeout */
      logger.msg(INFO, "start_reading_ftp: timeout waiting for modification_time");
      globus_ftp_client_abort(&ftp_handle);
      cond.wait(mutex);
      //globus_ftp_client_handle_flush_url_state(&ftp_handle,url.CanonicalURL().c_str());
      //buffer->error_read(true);
      ...
      logger.msg(INFO, "start_reading_ftp: failed to get file's modification time");
      //buffer->error_read(true);
      // DataPointDirect::stop_reading();
      //return false;
    } else {
  ...
  if(limit_length) {
    ...
    res=globus_ftp_client_get(&ftp_handle,url.CanonicalURL().c_str(),&ftp_opattr,
...
bool DataPointGridFTP::stop_reading(void) {
  ...
  cond.wait(mutex); 
  logger.msg(DEBUG, "stop_reading_ftp: exiting: %s", url.CanonicalURL().c_str());
  globus_ftp_client_handle_flush_url_state(&ftp_handle,url.CanonicalURL().c_str());
  return true;
}
  ...
  for(;;) {
    ...
    if(!it->buffer->for_read(h,l,true)) { /* eof or error */
      if(it->buffer->error()) { /* error -> abort reading */
        logger.msg(DEBUG, "ftp_read_thread: for_read failed - aborting: %s", it->url.CanonicalURL().c_str());
        globus_ftp_client_abort(&(it->ftp_handle));
      };
      break;
    };
    ...
    if(res != GLOBUS_SUCCESS) {
      logger.msg(VERBOSE, "ftp_read_thread: Globus error: %s", GlobusResult(res).str().c_str());
      ...
      if(registration_failed>=10) {
        ...
        logger.msg(VERBOSE, "ftp_read_thread: too many registration failures - abort: %s", it->url.CanonicalURL().c_str());
      } else {
        logger.msg(VERBOSE, "ftp_read_thread: failed to register globus buffer - will try later: %s", it->url.CanonicalURL().c_str());
...
void DataPointGridFTP::ftp_read_callback(void* arg,globus_ftp_client_handle_t* handle,globus_object_t* error,globus_byte_t* buffer,globus_size_t length,globus_off_t offset,globus_bool_t eof) {
  DataPointGridFTP* it = (DataPointGridFTP*)arg;
  if(error != GLOBUS_SUCCESS) {
...
void DataPointGridFTP::ftp_get_complete_callback(void* arg,globus_ftp_client_handle_t* handle,globus_object_t* error) {
...
bool DataPointGridFTP::start_writing(DataBufferPar &buf,DataCallback *space_cb) {
...
/* !!!! TODO - preallocate here */
  ...
  if(!no_checks) {
    ...
    if(!DataPointGridFTP::mkdir_ftp()) {
      logger.msg(DEBUG, "start_writing_ftp: mkdir failed - still trying to write");
    };
  };
  logger.msg(DEBUG, "start_writing_ftp: put");
  if(limit_length) {
    res=globus_ftp_client_partial_put(&ftp_handle,url.CanonicalURL().c_str(),&ftp_opattr,
                          GLOBUS_NULL,range_start,range_start+range_length,&ftp_put_complete_callback,this);

  } else {
    res=globus_ftp_client_put(&ftp_handle,url.CanonicalURL().c_str(),&ftp_opattr,
...
void DataPointGridFTP::ftp_write_callback(void* arg,globus_ftp_client_handle_t* handle,globus_object_t* error,globus_byte_t* buffer,globus_size_t length,globus_off_t offset,globus_bool_t eof) {
  DataPointGridFTP* it = (DataPointGridFTP*)arg;
...
void DataPointGridFTP::ftp_put_complete_callback(void* arg,globus_ftp_client_handle_t* handle,globus_object_t* error) {
...
bool DataPointGridFTP::list_files(std::list<DataPoint::FileInfo> &files,bool resolve) {
  if(!DataPointDirect::list_files(files,resolve)) return false;
  Lister lister; 
  if(lister.retrieve_dir(url.CanonicalURL()) != 0) {
    logger.msg(ERROR, "Failed to obtain listing from ftp: %s", url.CanonicalURL().c_str());
    return false;
  };
  ...
  for(std::list<ListerFile>::iterator i = lister.begin();i!=lister.end();++i) {
    ...
    if(resolve) {
      ...
      if(i->CheckSize()) {
        ...
        logger.msg(VERBOSE, "list_files_ftp: looking for size of %s", f_url.c_str());
        ...
        if(res != GLOBUS_SUCCESS) {
          ...
          logger.msg(INFO, "Globus error %s", GlobusResult(res).str().c_str()); result=false;
        } else if(!cond.timed_wait(mutex, x(300))) { /* 5 minutes timeout */
          logger.msg(INFO, "list_files_ftp: timeout waiting for size");
          globus_ftp_client_abort(&ftp_handle);
          cond.wait(mutex); result=false;
        } else if(condstatus != 0) {
      ...
      if(i->CheckCreated()) {
        ...
        logger.msg(VERBOSE, "list_files_ftp: looking for modification time of %s", f_url.c_str());
        res = globus_ftp_client_modification_time(&ftp_handle,f_url.c_str(),
                     &ftp_opattr,&gl_modify_time,&ftp_complete_callback,this);
        if(res != GLOBUS_SUCCESS) {
          logger.msg(DEBUG, "list_files_ftp: globus_ftp_client_modification_time failed");
          logger.msg(INFO, "Globus error: %s", GlobusResult(res).str().c_str()); result=false;
	} else if(!cond.timed_wait(mutex, x(300))) { /* 5 minutes timeout */
          logger.msg(INFO, "list_files_ftp: timeout waiting for modification_time");
          globus_ftp_client_abort(&ftp_handle);
          cond.wait(mutex); result=false;
        } else if(condstatus != 0) {
          logger.msg(INFO, "list_files_ftp: failed to get file's modification time");
          result=false;
        } else {
          GlobusTimeAbstimeGet(gl_modify_time,modify_time,modify_utime);
...
DataPointGridFTP::DataPointGridFTP(const URL& url) : DataPointDirect(url), ftp_active(false) {}
  ...
  if(!DataPointDirect::init_handle()) return false;
    ...
    if(!ftp_active) {
      ...
      if((res=globus_ftp_client_operationattr_init(
        ...
        logger.msg(ERROR, "init_handle: globus_ftp_client_operationattr_init failed");
        logger.msg(ERROR, "Globus error: %s", GlobusResult(res).str().c_str());
