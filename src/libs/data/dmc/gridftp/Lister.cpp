...
bool ListerFile::SetAttributes(const char* facts) {
  ...
  for(;*p;) {
    ...
    if(((value-name-1) == 4) && (strncasecmp(name,"type",4)==0)) {
      ...
      size=stringtoi(tmp_s); size_available=true;
    } else if(((value-name-1) == 6) && (strncasecmp(name,"modify",6)==0)) {
      std::string tmp_s(value,(int)(p-value));
      created=stringtoi(tmp_s); created_available=true;
    };
  };
  return true;
}
    ...
    if(wait_for_response) {
      ...
      if((sresp) && (resp_n>0)) {
        if(delim == 0) {
          ...
          else { logger.msg(ERROR, "Memory allocation error"); };
        } 
        else {
...
int Lister::retrieve_dir(const std::string &url) {
    ...
    if(globus_url_parse(url.c_str(),&url_) != GLOBUS_URL_SUCCESS) {
      logger.msg(ERROR, "Failed parsing url %s", url.c_str());
      return -1;
    };
    if((url_.scheme_type != GLOBUS_URL_SCHEME_FTP) && 
       (url_.scheme_type != GLOBUS_URL_SCHEME_GSIFTP)) { 
      logger.msg(ERROR, "Unsupported protocol in url %s", url.c_str());
    ...
    if(reconnect) { 
      ...
      */
      ...
      if(wait_for_callback() != CALLBACK_DONE) {
        logger.msg(ERROR, "Failed to connect to server %s:%d", host, port);
        resp_destroy();
        return -1;
      };
      resp_destroy();
    ...
    if(url_.scheme_type == GLOBUS_URL_SCHEME_GSIFTP) {
      ...
      if((cmd_resp != GLOBUS_FTP_POSITIVE_COMPLETION_REPLY) &&
        ...
        if(sresp) {
          logger.msg(INFO, "DCAU failed: %s", sresp); free(sresp);
        } else { logger.msg(INFO, "DCAU failed"); };
        return -1;
      };
      free(sresp);
    };
    ...
    if(cmd_resp == GLOBUS_FTP_POSITIVE_COMPLETION_REPLY) {
      ...
      if(sresp) {
        ...
        free(sresp);
    } else { logger.msg(INFO, "NLST/MLSD failed"); };
      return -1;
    };
    free(sresp);
    /* start transfer */
...
/* waiting for response received */
      ...
      if((cmd_resp != GLOBUS_FTP_POSITIVE_PRELIMINARY_REPLY) &&
        ...
        if(sresp) {
          ...
          free(sresp);
        } else { logger.msg(INFO, "Data transfer aborted"); };
        /*
          !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
