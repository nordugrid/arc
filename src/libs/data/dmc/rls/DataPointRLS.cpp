...
DataPointRLS::DataPointRLS(const URL& url) : DataPointIndex(url), guid_enabled(false) {
...
bool DataPointRLS::meta_resolve_callback(globus_rls_handle_t* h,const URL& url,void* arg) {
...
//  string& lfn = ((meta_resolve_rls_t*)arg)->lfn;
  ...
  if(it.guid_enabled && source && !arg_->guid.length()) {
    ...
    if(err != GLOBUS_SUCCESS) {
      ...
      logger.msg(INFO, "Failed to find GUID for specified LFN in %s: %s", url.str().c_str(), errmsg);
      return true;
    };
    if(!guids) {
      logger.msg(INFO, "There is no GUID for specified LFN in %s", url.str().c_str());
      return true;
    };
  ...
  if(err != GLOBUS_SUCCESS) {
    ...
    if(errcode == GLOBUS_RLS_INVSERVER) {
      ...
      logger.msg(INFO, "Warning: can't get PFNs from server %s: %s", url.str().c_str(), errmsg);
      return true;
    };
  };
  if(!(arg_->success)) {
  ...
  if(arg_->locations_empty) {
    ...
    for(list_p=pfns_list;list_p;list_p=globus_list_rest(list_p)) {
      ...
      std::list<DataPointRLS::Location>::iterator loc =
        it.locations.insert(it.locations.end(),DataPointRLS::Location(str2->s2,pfn));
      loc->arg=(void*)1; // marker
      logger.msg(DEBUG, "Adding location: %s - %s", str2->s2, pfn.str().c_str());
    };
  } else {
  ...
  if(!arg_->obtained_info) {
    ...
    if(err != GLOBUS_SUCCESS) {
      ...
      logger.msg(INFO, "Warning: Failed to obtain attributes from %s: %s", url.str().c_str(), errmsg);
      return true;
    };
    ...
    for(globus_list_t* list_pa=attr_list;list_pa;
      ...
      if(strcmp(attr->name,"filechecksum") == 0) {
        ...
        if(!it.meta_created_valid) {
          ...
          if(created == -1) created.SetTime(stringtoull(attr->val.s));
...
bool DataPointRLS::meta_resolve(bool source) {
  ...
  if(source) {
    ...
    for(;loc!=locations.end();) {
      if(loc->arg==NULL) {
        logger.msg(DEBUG, "Removing location: %s - %s", loc->meta.c_str(), loc->url.str().c_str());
        loc=locations.erase(loc);
      } else {
        logger.msg(DEBUG, "Using location: %s - %s", loc->meta.c_str(), loc->url.str().c_str());
        loc->arg=(void*)1; ++loc;
      };
    };
  } else { 
    ...
    lrcs.push_back(url.ConnectionURL());
    if(locations.size() == 0) {
      logger.msg(INFO, "Locations are missing in destination RLS url - will use those registered with special name");
    };
    meta_resolve_rls_t arg(*this,source);
    bool res = rls_find_lrcs(rlis,lrcs,true,false,&meta_resolve_callback,(void*)(&arg));
    if(!arg.success) return false;
    ...
    for(;loc!=locations.end();) {
      ...
      if(loc->url.Path() == "se") { u+="?"; se_uses_lfn=true; }
      else { u+="/"; };
      if(guid_enabled) {
        std::string guid = GUID();
        if((!se_uses_lfn) && (!pfn_path.empty())) { u+=pfn_path; }
        else { u+=guid; };
      } else {
        if((!se_uses_lfn) && (!pfn_path.empty())) { u+=pfn_path; }
        else { u=url.Path(); };
  ...
  logger.msg(DEBUG, "meta_get_data: created: %s", meta_created().str().c_str());
...
bool DataPointRLS::meta_postregister(bool replication,bool failure) {
  ...
  std::string pfn;
  std::string guid;
  pfn = location->url.CanonicalURL(); // it is always better to register pure url
...
;
        if((errcode != GLOBUS_RLS_LFN_NEXIST) &&
          ...
          logger.msg(INFO, "Failed to check for existing LFN in %s: %s", url.str().c_str(), errmsg);
          globus_rls_client_close(h);
          return false;
        };
      };
  ...
  if(err != GLOBUS_SUCCESS) {
    ...
    if(errcode != GLOBUS_RLS_ATTR_EXIST) {
      logger.msg(INFO, "Warning: failed to add attribute to RLS: %s", errmsg);
    };
  };
  if(meta_size_valid) {
    attr.name="size";
...
bool DataPointRLS::meta_unregister_callback(globus_rls_handle_t* h,const URL& url,void* arg) {
  ...
  if(it.guid_enabled && !arg_->guid.length()) {
    ...
    if(err != GLOBUS_SUCCESS) {
      globus_rls_client_error_info(err,&errcode,errmsg,MAXERRMSG,GLOBUS_FALSE);
      logger.msg(VERBOSE, "Failed to find GUID for specified LFN in %s: %s", url.str().c_str(), errmsg);
      return true;
    };
    if(!guids) {
      logger.msg(VERBOSE, "There is no GUID for specified LFN in %s", url.str().c_str());
      return true;
    };
    globus_rls_attribute_object_t* obattr =
                 (globus_rls_attribute_object_t*)globus_list_first(guids);
  ...
  if(arg_->all) {
    err = globus_rls_client_lrc_get_pfn(h,(char*)lfn.c_str(),&lrc_offset,lrc_limit,&pfns_list);
    if(err != GLOBUS_SUCCESS) {
      globus_rls_client_error_info(err,&errcode,errmsg,MAXERRMSG,GLOBUS_FALSE);
      logger.msg(INFO, "Warning: Failed to retrieve LFN/PFNs from %s: %s", url.str().c_str(), errmsg);
    ...
    for(list_p=pfns_list;list_p;list_p=globus_list_rest(list_p)) {
      ...
      if(pfn.Protocol() == "se") {
        ...
        err = globus_rls_client_lrc_delete(h,(char*)lfn.c_str(),(char*)pfn.str().c_str());
        if(err != GLOBUS_SUCCESS) {
          globus_rls_client_error_info(err,&errcode,errmsg,MAXERRMSG,GLOBUS_FALSE);
          if((errcode != GLOBUS_RLS_MAPPING_NEXIST) && 
             (errcode != GLOBUS_RLS_LFN_NEXIST) && 
             (errcode != GLOBUS_RLS_PFN_NEXIST)) {
            logger.msg(INFO, "Warning: Failed to delete LFN/PFN from %s: %s", url.str().c_str(), errmsg);
    ...
    err = globus_rls_client_lrc_delete(h,(char*)lfn.c_str(),(char*)(it.location->url.str().c_str()));
    if(err != GLOBUS_SUCCESS) {
      globus_rls_client_error_info(err,&errcode,errmsg,MAXERRMSG,GLOBUS_FALSE);
      if((errcode != GLOBUS_RLS_MAPPING_NEXIST) &&
         (errcode != GLOBUS_RLS_LFN_NEXIST) &&
...
bool DataPointRLS::meta_unregister(bool all) {
  ...
  int lrc_offset = 0;
  int lrc_limit = 0;
  err = globus_rls_client_rli_get_lrc(h,(char*)(url.Path().c_str()),&lrc_offset,lrc_limit,&lrcs);
  if(err != GLOBUS_SUCCESS) {
    globus_rls_client_error_info(err,&errcode,errmsg,MAXERRMSG,GLOBUS_FALSE);
    if(errcode == GLOBUS_RLS_LFN_NEXIST) {
  ...
  for(p=lrcs;p;p=globus_list_rest(p)) {
    ...
    if(lrc) {
      ...
      if(err != GLOBUS_SUCCESS) {
        globus_rls_client_error_info(err,&errcode,errmsg,MAXERRMSG,GLOBUS_FALSE);
        logger.msg(INFO, "Warning: Failed to connect to LRC at %s: %s", lrc, errmsg);
        failure=true; continue;
      };
    } else { h_=h; }; // This server is already connected
    if(all) {
      err = globus_rls_client_lrc_get_pfn(h_,(char*)(url.Path().c_str()),&lrc_offset,lrc_limit,&pfns_list);
      if(err != GLOBUS_SUCCESS) {
        globus_rls_client_error_info(err,&errcode,errmsg,MAXERRMSG,GLOBUS_FALSE);
      ...
      for(list_p=pfns_list;list_p;list_p=globus_list_rest(list_p)) {
        ...
        if(pfn.Protocol() == "se") {
          ...
          err = globus_rls_client_lrc_delete(h_,(char*)(url.Path().c_str()),(char*)pfn.str().c_str());
          if(err != GLOBUS_SUCCESS) {
            globus_rls_client_error_info(err,&errcode,errmsg,MAXERRMSG,GLOBUS_FALSE);
            if((errcode != GLOBUS_RLS_MAPPING_NEXIST) && (errcode != GLOBUS_RLS_LFN_NEXIST) && (errcode != GLOBUS_RLS_PFN_NEXIST)) {
              logger.msg(INFO, "Warning: Failed to delete LFN/PFN from %s: %s", lrc?lrc:url.ConnectionURL().c_str(), errmsg);
              if(lrc) globus_rls_client_close(h_);
              failure=true; continue;
            };
          };
      ...
      err = globus_rls_client_lrc_delete(h_,(char*)(url.Path().c_str()),(char*)(location->url.str().c_str()));
      if(err != GLOBUS_SUCCESS) {
        globus_rls_client_error_info(err,&errcode,errmsg,MAXERRMSG,GLOBUS_FALSE);
        if((errcode != GLOBUS_RLS_MAPPING_NEXIST) && 
           (errcode != GLOBUS_RLS_LFN_NEXIST) &&
           (errcode != GLOBUS_RLS_PFN_NEXIST)) {
          logger.msg(INFO, "Warning: Failed to delete LFN/PFN from %s: %s", lrc, errmsg);
          if(lrc) globus_rls_client_close(h_);
          failure=true; continue;
        };
      };
...
static bool get_attributes(globus_rls_handle_t* h,const std::string& lfn,DataPoint::FileInfo &f) {
  globus_list_t* attr_list;
  char errmsg[MAXERRMSG];
  globus_result_t err;
  int errcode;
 ...
 if(err != GLOBUS_SUCCESS) {
   globus_rls_client_error_info(err,&errcode,errmsg,MAXERRMSG,GLOBUS_FALSE);
   if(errcode != GLOBUS_RLS_ATTR_NEXIST) {
     // logger.msg(INFO, "Warning: Failed to retrieve attributes: %s", errmsg);
     return false;
   };
   return true;
 };
 for(globus_list_t* pa=attr_list;pa;pa=globus_list_rest(pa)) {
   ...
   if(strcmp(attr->name,"filechecksum") == 0) {
     ...
     f.size=stringtoi(attr->val.s); f.size_available=true;
   } else if(strcmp(attr->name,"modifytime") == 0) {
     Time created(attr->val.s);
     if(created == -1) created.SetTime(stringtoull(attr->val.s));
     f.created=created; f.created_available=true;
   } else if(strcmp(attr->name,"created") == 0) {
     Time created(attr->val.s);
     if(created == -1) created.SetTime(stringtoull(attr->val.s));
     f.created=created; f.created_available=true;
...
bool DataPointRLS::list_files_callback(globus_rls_handle_t* h,const URL& url,void* arg) {
  ...
  if(err != GLOBUS_SUCCESS) {
    ...
    logger.msg(INFO, "Failed to retrieve list of LFNs/PFNs from %s", url.str().c_str());
    return true;
  };
  ((list_files_rls_t*)arg)->success=true;
  std::string last_lfn = "";
  ...
  for(p=pfns;p;p=globus_list_rest(p)) {
    ...
    if(it.guid_enabled) {
      if(lfn != last_guid) {
        ...
        err = globus_rls_client_lrc_attr_value_get(h,(char*)lfn.c_str(),"lfn",
      ...
      if(last_lfn.length()) {
        logger.msg(DEBUG, "lfn: %s(%s) - %s", last_lfn.c_str(), last_guid.c_str(), pfn.str().c_str());
