...
bool DataPointLFC::meta_resolve(bool source) {
  ...
  if(lfc_startsess(const_cast<char*>((url.Host() + ':' + tostring(url.Port())).c_str()),"ARC") != 0) return false;
  int nbentries = 0;
  ...
  if(locations.size() == 0) {
    for(int n=0;n<nbentries;n++) {
      ...
      logger.msg(DEBUG, "Adding location: %s - %s", url.ConnectionURL().c_str(), entries[n].sfn);
    };
  } else {
    ...
    for(;loc!=locations.end();++loc) {
      ...
      for(int n=0;n<nbentries;n++) {
        if(strncmp(entries[n].sfn,loc->meta.c_str(),loc->meta.length())==0){
          logger.msg(DEBUG, "Adding location: %s - %s", url.ConnectionURL().c_str(), entries[n].sfn);
          if(source) {
            loc->meta=url.ConnectionURL(); loc->url=std::string(entries[n].sfn);
          } else {
...
bool DataPointLFC::meta_preunregister(bool replication) {
  ...
  if(lfc_unlink(url.Path().c_str()) != 0) {
    if((serrno != ENOENT) && (serrno != ENOTDIR)) {
      logger.msg(ERROR, "Failed to remove LFN in LFC - You may need to do that by hand");
      lfc_endsess();
      return false;
    };
  };
...
bool DataPointLFC::meta_unregister(bool all) {
  ...
  if(all) {
    ...
    if(lfc_delreplica(guid.c_str(),NULL,location->url.str().c_str()) != 0) {
      lfc_endsess();
      logger.msg(ERROR, "Failed to remove location from LFC");
      return false;
    };
...
bool DataPointLFC::list_files(std::list<FileInfo> &files,bool resolve) {
  if(lfc_startsess(const_cast<char*>((url.Host() + ':' + tostring(url.Port())).c_str()),"ARC") != 0) return false;
  struct lfc_filestatg st;
  if(lfc_statg(url.Path().c_str(),NULL,&st) != 0) { 
    lfc_endsess();
  ...
  f->created=st.mtime;
  f->type=(st.filemode & S_IFDIR)?FileInfo::file_type_dir:FileInfo::file_type_file;
  int nbentries = 0;
  struct lfc_filereplica* entries = NULL;
  if(lfc_getreplica(url.Path().c_str(),NULL,NULL,&nbentries,&entries) == 0) {
    for(int n=0;n<nbentries;n++) {
