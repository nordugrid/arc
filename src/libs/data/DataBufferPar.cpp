...
bool DataBufferPar::set(CheckSum *cksum,unsigned int size,int blocks) {
  lock.lock();
  if(blocks < 0) { lock.unlock(); return false; };
  if(bufs != NULL) {
...
bool DataBufferPar::for_read(int &handle,unsigned int &length,bool wait) {
  lock.lock();
  if(bufs == NULL) { lock.unlock(); return false; };
  for(;;) {
    ...
    if(!wait) { lock.unlock(); return false; };
    if(!cond_wait()) { lock.unlock(); return false; };
  };
  lock.unlock();
  return false;
}
...
bool DataBufferPar::is_read(int handle,unsigned int length,unsigned long long int offset) {
  lock.lock();
  if(bufs == NULL) { lock.unlock(); return false; };
  if(handle >= bufs_n) { lock.unlock(); return false; };
  if(!bufs[handle].taken_for_read) {lock.unlock(); return false;};
  if(length>bufs[handle].size) { lock.unlock(); return false; };
...
bool DataBufferPar::is_written(int handle) {
  ...
  if(bufs == NULL) { lock.unlock(); return false; };
  if(handle >= bufs_n) { lock.unlock(); return false; };
  if(!bufs[handle].taken_for_write) {lock.unlock();return false;};
  /* speed control */
...
char* DataBufferPar::operator[](int block) {
  lock.lock();
  if((block<0) || (block>=bufs_n)) { lock.unlock(); return NULL; };
