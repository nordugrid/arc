...
bool DataSpeed::verbose(void) { return be_verbose; };

void DataSpeed::verbose(bool val) { be_verbose=val; };
...
bool DataSpeed::transfer(unsigned long long int n) {
  if(disabled) { last_time=time(NULL); return true; };
  time_t t = time(NULL);
  time_t dt= t - last_time;
  Nall+=n;
  if(dt > T) { N=(n*dt)/T; }
  else { N=(N*(T-dt))/T+n; };
  if((t-first_time) >= T*3) { /* make decision only after statistics settles */
    /* check for speed */
    if( N < (T*min_speed) ) {
  ...
  return !(min_speed_failed || min_average_speed_failed || max_inactivity_time_failed);
}
...
void DataSpeed::print_statistics(FILE *o,time_t t) {
  if(show_progress != NULL) {
    (*show_progress)(o,
          ...
          Nall,Nmax,
          (t>first_time? 
            (((double)N)/(((t-first_time)>T) ? T : (t-first_time))) : 
            ((double)0)
          ),
          ( t>first_time ? 
  ...
  fprintf(o,"%s%5u s: %10.1f kB  %8.1f kB/s  %8.1f kB/s    %c %c %c       \n",
          verbose_prefix.c_str(),
          (unsigned int)(t-first_time),
          ((double)Nall)/1024,
          (t>first_time? 
            (((double)N)/(((t-first_time)>T) ? T : (t-first_time))/1024) : 
            ((double)0)
          ),
          ( t>first_time ? 
...
void DataSpeed::set_min_speed(unsigned long long int min_speed_,time_t min_speed_time_) {
  min_speed=min_speed_; min_speed_time=min_speed_time_;
}

void DataSpeed::set_min_average_speed(unsigned long long int min_average_speed_) {
...
DataSpeed::DataSpeed(unsigned long long int min_speed_,time_t min_speed_time_,
            unsigned long long int min_average_speed_,
            time_t max_inactivity_time_,time_t base_) {
  min_speed=min_speed_; min_speed_time=min_speed_time_;
  min_average_speed=min_average_speed_;
  max_inactivity_time=max_inactivity_time_;
  T=base_; be_verbose=false; disabled=false;
  show_progress=NULL; Nmax=0;
  reset();
}

DataSpeed::~DataSpeed(void) {
  /* statistics to screen */
  if(be_verbose) print_statistics(stderr,time(NULL));
}

DataSpeed::DataSpeed(time_t base_) {
  min_speed=0; min_speed_time=0;
  min_average_speed=0;
