...
DataPointIndex::DataPointIndex(const URL& url) :
        DataPoint(url),is_metaexisting(false),is_resolved(false) {
...
bool DataPointIndex::get_info(DataPoint::FileInfo &fi) {
  if(!meta_resolve(true)) { return false; }; // resolution failure
  fi.name=url.Path();
  for(std::list<DataPointIndex::Location>::iterator i = locations.begin();
                                              i!=locations.end();++i) {
...
bool DataPointIndex::have_locations(void) const {
  if(!url) return false;
  return (locations.size() != 0);
}

bool DataPointIndex::have_location(void) const {
  if(!url) return false;
...
bool DataPointIndex::next_location(void) {
  if(tries_left <= 0) return false;
...
bool DataPointIndex::remove_location(void) {
  if(location == locations.end()) return false;
  location=locations.erase(location);
  return true;
}
...
bool DataPointIndex::add_location(const std::string& meta_loc,const URL& loc) {
...
const URL& DataPoint::base_url(void) const {
  return url;
}
