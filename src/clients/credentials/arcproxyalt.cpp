...
static bool contact_voms_servers(std::list<std::string>& vomslist, std::list<std::string>& orderlist,
      bool use_gsi_comm, bool use_http_comm, const std::string& voms_period, Arc::UserConfig& usercfg,
      Arc::Logger& logger, AuthN::Credentials& signer, AuthN::ProxyCredentialsRequest& proxyreq);
...
static bool contact_myproxy_server(const std::string& myproxy_server, const std::string& myproxy_command,
      ...
      const std::string& proxy_path, Arc::UserConfig& usercfg, Arc::Logger& logger);
...
int main(int argc, char *argv[]) {
  ...
  if(use_nssdb) {
    ...
    ctx.SetCredentials(proxy_path, "");
    AuthN::Credentials proxy_cred(ctx);
    Arc::Time left = proxy_cred.GetValidTill();
  ...
  if (info) {
    ...
    // EmptyContext is used , and then cert/key and CA paths
    // are set manually. AuthN::Context will only not use
    // cert/key, if both cert/key and proxy paths are set
    AuthN::Context ctx(AuthN::Context::EmptyContext);
    AuthN::Context full_ctx(AuthN::Context::ClientFullContext);
    ctx.SetCredentials(full_ctx.GetCertPath(), full_ctx.GetKeyPath());
    ctx.SetCAPath(full_ctx.GetCAPath());
    AuthN::Credentials holder(ctx);
    std::cout << Arc::IString("Subject: %s", holder.GetSubjectName()) << std::endl;
    ...
    Arc::Time end(holder.GetValidTill());
    Arc::Time start(holder.GetValidFrom());
    if (end < now)
      ...
      std::cout << Arc::IString("Time left for proxy: %s", (end - now).istr()) << std::endl;
    if(!ctx.GetCertPath().empty())
      std::cout << Arc::IString("Proxy path: %s", ctx.GetCertPath()) << std::endl;
    ...
    std::string cert_str; holder.GetCertificate(cert_str);
    res = parseVOMSAC(cert_str, ca_dir, "", voms_dir, voms_trust_dn, voms_attributes, true, true);
  ...
  try {
    AuthN::Context ctx(AuthN::Context::EmptyContext);
    AuthN::Context empty_ctx(AuthN::Context::EmptyContext);
    AuthN::Context full_ctx(AuthN::Context::ClientFullContext);
    ctx.SetCredentials(full_ctx.GetCertPath(), full_ctx.GetKeyPath());
    ctx.SetCAPath(full_ctx.GetCAPath());

    AuthN::Credentials signer(ctx); 
    ...
    signer.GetPrivateKey(privk_str);
    if(privk_str.empty()) {
    ...
    Arc::Time end(signer.GetValidTill());
    Arc::Time start(signer.GetValidFrom());
    if (now > end) {
    ...
    int keybits = 1024;
    AuthN::ProxyCredentialsRequest proxyreq(empty_ctx);
    proxyreq.MakeKeys(keybits);
    proxyreq.MakeRequest();
    proxyreq.GetPrivateKey(private_key);

    if(vomslist.size() > 0)
      contact_voms_servers(vomslist, orderlist, use_gsi_comm, use_http_comm, voms_period, usercfg, logger, signer, proxyreq);

    proxyreq.SetValidFrom(proxy_start.GetTime());
    proxyreq.SetValidTill((proxy_start + proxy_period).GetTime());
    AuthN::Credentials::Extension policy;
    std::string policy_source = constraints["proxyPolicy"].empty() ? constraints["proxyPolicyFile"] : constraints["proxyPolicy"];
    std::string policy_string = get_proxypolicy(policy_source);
    if(!policy_string.empty()) {
      policy.value = policy_string;
      proxyreq.SetPolicy(policy);
    }

    AuthN::Credentials proxy(empty_ctx);
    AuthN::Status stat = signer.Sign(proxyreq, proxy);
    if(stat != AuthN::Status(0))
      throw std::runtime_error("Failed to sign proxy");

    std::string proxy_cert;
    proxy.GetCertificate(proxy_cert);
    signer.GetCertificate(signing_cert);
    signer.GetChain(signing_cert_chain);
    ...
    Arc::Time left = proxy.GetValidTill();
    std::cout << Arc::IString("Proxy generation succeeded") << std::endl;
    std::cout << Arc::IString("Your proxy is valid until: %s", left.str(Arc::UserTime)) << std::endl;
...
static bool contact_voms_servers(std::list<std::string>& vomslist, std::list<std::string>& orderlist, 
  ...
  AuthN::Context ctx(AuthN::Context::EmptyContext);
  AuthN::Credentials tmp_proxy(ctx);
  AuthN::ProxyCredentialsRequest tmp_proxyreq(ctx);

  int keybits = 1024;
  tmp_proxyreq.MakeKeys(keybits);
  tmp_proxyreq.MakeRequest();
  Arc::Time now;
  Arc::Period period = 3600 * 12;
  tmp_proxyreq.SetValidFrom((now-Arc::Period(300)).GetTime());
  tmp_proxyreq.SetValidTill((now + period + 300).GetTime());

  std::string proxy_private_key, proxy_cert, signing_cert, signing_cert_chain;
  tmp_proxyreq.GetPrivateKey(proxy_private_key);
  signer.GetCertificate(signing_cert);
  signer.GetChain(signing_cert_chain);

  AuthN::Status stat = signer.Sign(tmp_proxyreq, tmp_proxy);
  if(stat != AuthN::Status(0))
    throw std::runtime_error("Failed to sign proxy");

  tmp_proxy.GetCertificate(proxy_cert);
  ...
  //If the location is a directory, all the files and directories will be scanned
...
static std::string get_cert_dn(const std::string& cert_file) {
  std::string dn_str;
  AuthN::Context ctx(AuthN::Context::EmptyContext);
  ctx.SetCredentials(cert_file, "");
  AuthN::Credentials cert(ctx);
