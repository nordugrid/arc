...
sub get_pbs_version ($) {
    ...
    if ( $? != 0 ) {
        error("Can't run qmgr");
    }
    if ($qmgr_string =~ /pbs_version = \b(\D+)_(\d\S+)\b/) {
        $lrms_type = $1;
        $lrms_version = $2;
    } else {
        error("Cannot determine PBSPro version from qmrg. Exiting...");
    }
}
...
#
# According to PBSPro manual:
# Format: exec_host=<host A>/<index>*<CPUs> [+<host B>/<index> * <CPUs>] where index is task slot number starting at 0, on that host, and CPUs is the number of CPUs assigned to the job, 1 if omitted.
#
sub split_hostlist {
...
# Get number of requested cpus from the values of Resource_List.select
sub set_cpucount {
    my ($job) = (@_);
    my $select = $job->{"Resource_List.select"};
    my $totalcpus = 0;
    for my $chunk (split '\+', $select) {
        my $cpus = cpus_in_select_chunk($chunk);
        $totalcpus += $cpus;
    }
    $job->{cpus} = $totalcpus;
