...
my $suite = new InfoproviderTestSuite('slurm');
...
args="sinfo -a -h -o PartitionName=%P TotalCPUs=%C TotalNodes=%D MaxTime=%l DefTime=%L NodeList=%N"
...
ENDSIMULATOROUTPUT
  ...
  is($lrms_info->{queues}{queue1}{nodes}, undef, 'queue1->nodes');
  
  is(ref $lrms_info->{queues}{queue1}{users}, 'HASH', 'has users');
  is(ref $lrms_info->{queues}{queue1}{users}{user1}, 'HASH', 'has user1');
  #is_deeply($lrms_info->{queues}{queue1}{users}{user1}{freecpus}, { '1' => undef }, 'user1->freecpus');
