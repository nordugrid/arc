...
Arc::MCC_Status ARexService::MigrateActivity(ARexGMConfig& config,Arc::XMLNode in,Arc::XMLNode out,const std::string& clientid) {
  ...
  */
  ...
  Arc::XMLNode migrateid = in["ActivityIdentifier"];
  if(!migrateid) {
    // Wrongly formated request
    logger_.msg(Arc::ERROR, "MigrateActivity: no migration id found");
    Arc::SOAPFault fault(out.Parent(),Arc::SOAPFault::Sender,"Can't find MigrattionID element in request");
    InvalidRequestMessageFault(fault,"ActivityIdentifier","Element is missing");
    out.Destroy();
