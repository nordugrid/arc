...
parse_arg_file () {
   ...
   joboption_globalgridid="gsiftp://${joboption_rsl_hostname}:2811/jobs/${joboption_gridid}"
}
...
add_user_env () {
   ...
   #set gridid
   #echo "export joboption_globalgridid=${joboption_globalgridid}" >> $LRMS_JOB_SCRIPT
...
move_files_to_node () {
  ...
  cat >> $LRMS_JOB_SCRIPT <<'EOSCR'
  if [ ! -z "$RUNTIME_LOCAL_SCRATCH_DIR" ] ; then #add here? && [ ! -z "$RUNTIME_NODE_SEES_FRONTEND" ]
...
move_files_to_frontend () {
  ...
  if [ ! -z "$RUNTIME_LOCAL_SCRATCH_DIR" ] ; then 
    if [ ! -z "$RUNTIME_FRONTEND_SEES_NODE" ] ; then
      ...
      find "$RUNTIME_NODE_JOB_DIR" -mindepth 1 -maxdepth 1 -exec mv '{}' "$RUNTIME_FRONTEND_JOB_DIR" \;
      rm -rf "$RUNTIME_NODE_JOB_DIR"
    fi
  fi
