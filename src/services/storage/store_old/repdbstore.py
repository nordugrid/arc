...
from storage.common import rep_ahash_uri
...
try:
    DB_EID_INVALID   = db.DB_EID_INVALID
    DB_EID_BROADCAST = db.DB_EID_BROADCAST
except:
    # bsddb ver. < 4.7.5 does not define DB_EID_*
    # this is just an ugly hack for now
    DB_EID_INVALID = -2
    DB_EID_INVALID = -1
...
class RepClient(Client):
    
    def __init__(self, url, print_xml = False, ssl_config = {}):
        ns = self.NS_class('ahash', rep_ahash_uri)
        Client.__init__(self, url, ns, print_xml, ssl_config = ssl_config)

    def repMsg(self, msg):
        """ Call the repMsg method of the replicated A-Hash service.
            ...
            ('ahash:repSend', [
                ('ahash:changeRequestList', [
                    ('ahash:changeRequestElement', [
                        ('ahash:control', control),
                        ('ahash:rec', rec),
                        ('ahash:lsnp', lsnp),
                        ('ahash:envid', envid),
                        ('ahash:flags', flags),
                    ]) for changeID, (control, rec, lsnp, envid, flags) in msg.items()
                ])
            ])
        )
        msg = self.call(tree)
...
class RepMgr:
    """
    The replication manager class is in charge of the following tasks:
    - Initializing the list of known nodes
    - Providing the replication transport callback
    """
    
    def __init__(self, rep_client):
        self.masterID = DB_EID_INVALID
        self.replication_map = {}
        self.llock = threading.Lock()

    def beginRep(self, myID, dbenv, host_list):
        """
        Begin replication
        myID is the ID of this replica
        dbenv is the handle to the db environment
        host_list is a dict {ID, (URL, priority)}
        """
        self.myID = myID
        while not self.llock.acquire(False):
            time.sleep(0.2)
        # add hosts to replication map
        for ID, (url, priority) in host_list.items():
            pass
        

    def send(self, control, rec, lsnp, envid, flags):
        """
        internal send method for sending replication data
        ...
        transmitted by the send function.
        lsnp is a flag determining if rec contains an actual data entry. 
        Only relevant if flags contain DB_REP_PERMANENT
        envid is the environment ID of the destination node. If set to 
        DB_EID_BROADCAST, the message will be sent to every node that is
        known to local node
        ...
        DB_REP_PERMANENT
            ...
            integrity (for example, the message includes a transaction commit). 
            The application should take appropriate action to enforce the 
            reliability guarantees it has chosen, such as waiting for 
            acknowledgement from one or more clients.
        DB_REP_REREQUEST
        ...
        msg = {'repsend':[control, rec, lsnp, envid, flags]}

def repTransport(dbenv, control, rec, lsnp, envid, flags):
    """
    callback function for the replication api
    """
    mgr = RepMgr()
    mgr.send(msg)
