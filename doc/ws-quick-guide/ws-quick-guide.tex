...
number of grid projects, like NDGF, Swegrid or Swiss National Grid has
...
based on a service container -- the Hosting Environment Daemon (HED), and
different grid capabilities are implemented as Web Services residing in HED.

Currently included components are:
\begin{itemize}
\item an OGSA BES~\cite{ogsa-bes} compliant execution service -- \em{ARC
Resource-coupled EXecution service (A-REX)}
\item an echo service (for testing purposes)
\end{itemize}
...
from source or installing a precompiled binary requires different external
packages, furthermore the client and server packages have different dependencies
too. Below a list of the explicit requirements is shown:
...
You can get the latest source code for ARC1 from the Subversion repository. See
\url{http://svn.nordugrid.org} for more details.
...
with specific schema should be managed, see
\verb|$ARC_LOCATION/share/doc/arc/pdp_policy.xml.example| and 
\verb|$ARC_LOCATION/share/doc/arc/Policy.xsd| for details.
...
\texttt{/etc/grid-security/hostcert.pem} \\and\\
\texttt{/etc/grid-security/hostkey.pem}
...
Now you may use the command line utility ``\texttt{apinfo}'' to obtain a service
...
Listing~\ref{lst:apinfo} shows an example of proper output. In case of error see
\texttt{error.log} file and report problems to ARC development team. Include
content of \texttt{error.log} and server log.

\begin{lstlisting}[caption={Example of a good apinfo output},label=lst:apinfo]
...
Please note that you can run similar \texttt{apinfo} request against any ARC1
service except Echo service. Output should always be XML description of service.
...
NorduGrid Collaboration. Currently, the main contributor is the KnowARC project
(\url{www.knowarc.eu}), but the Collaboration is open to new members.
...
\url{download.nordugrid.org} or the Subversion code repository at
\url{svn.nordugrid.org}.
...
tracking system at \url{bugzilla.nordugrid.org}. For a more detailed
description, write access to the code repository and further
questions, write to the \texttt{nordugrid-discuss} mailing list (see
\url{www.nordugrid.org} for details). Ongoing and completed Grid Research
...
request tracking system available at \texttt{nordugrid-support@nordugrid.org}.
...
reported in the Bugzilla problem tracking system \url{bugzilla.nordugrid.org}.
...
\url{www.nordugrid.org}.

Contact information is kept updated on the \url{www.nordugrid.org} Web site.

\bibliography{ws-quick-guide}
\end{document}
