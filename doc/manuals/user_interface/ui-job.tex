...
Since  the  information  about  a job retrieved from a clsuter can be slightly out of date if the user very recently
submitted or removed a job, a warning is issued when this command is run. The \verb#--force# option disables this warning.

%If a specific cluster is selected for the syncronisation while the job list contains jobs  on  a  different  cluster,
%running  the arcsync command will remove the old jobs from the joblist, unless the \verb#--merge# option is given, in which
%case the new found jobs are merged with the jobs already in the joblist.
...
% \item[\texttt{FastestCPU}] -- ranks targets according to CPU performance
...
% \item[\texttt{SampleBroker}] -- Ranks targets randomly (written in Python)
\end{itemize}
