...
%%%%---- The title ----
{\centering \textsc{\Large The SWIG wrapped ARC Python API and the arcom utility package}\Large \par}
\vspace*{0.5cm}
...
\end{itemize}
\begin{itemize}
  \item{ \verb$datapoint_from_url(url_string, ssl_config = None)$\newline
  Creates DataPoint from specified URL (\verb$url_string$) with the 
  specified SSL configuration (if \verb$ssl_config$ is present).
    \begin{example}
