...
\hypersetup{
  pdfauthor = {D. Cameron, M. Ellert, J. J\"{o}nemo, A. Konstantinov, I. Marton, B. Mohn, J. K. Nilsen, M. Norden, W. Qiang, G. Roczei, O. Smirnova, F. Szalai, A. Waananen},
...
\vspace*{1.5cm}
%%%%---- A list of authors ----
    {\centering \large D. Cameron, M. Ellert, J. J\"{o}nemo, A. Konstantinov\footnote{aleksandr.konstantinov@fys.uio.no}, I. Marton, B. Mohn, J. K. Nilsen, M. N\'{o}rden, W. Qiang, G. Roczei, O. Smirnova, F. Szalai, A. W\"{a}\"{a}n\"{a}nen \large \par}
...
%Statements like "you are doing crap, I shall use Axis instead" are understood but not welcome and are usually reacted adequately.
...
The server side TCP MCC generates \texttt{PayloadStreamInterface} payload in the Message passed to the next MCC which can be used for communicating through open TCP channel. Currently it ignores any payload attached to the returned Message. That MCC also fills following Message Attributes in the incoming Message while passing to the next MCC:
\begin{enumerate}
