...
The server side TCP MCC generates \texttt{PayloadStreamInterface} payload in the Message passed to next MCC whihc can be used for communicationg through open TCP channel. Currently it ignores any payload attached to the returned Message. That MCC also fills following Message Attrbutes in incoming message while passing to next MCC:
\begin{enumerate}
