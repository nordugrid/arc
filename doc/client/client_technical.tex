...
The \texttt{Migrate} method behave in much the same way as the \texttt{Submit}
method, but is conceptually different from that method. The \texttt{Migrate}
method takes two additional arguments. A {\URL} \texttt{url} which is
the jobid of a job which should be migrated, and a boolean
\texttt{forcemigration} which will indicate if the job, specified in the
{\JobDescription} \texttt{jobdesc}, should persist if the cluster
({\ExecutionTarget}) does not succeed submitting a kill/terminate request
for the job specified by \texttt{url}. The \texttt{Migrate} method will
also fill the {\XMLNode} \texttt{info} variable with information about the
job for later job management, which additionally include the old jobid
(\texttt{url}) of the migrated job.
...
succeed submitting a kill/terminate request for the job. The integer
\texttt{timeout} argument have not yet been implemented.
