...
There are two usable broker type at the current state: \textbf{FastQueueBroker} and \textbf{RandomBroker}. These classes are inheritance of the "super \textbf{Broker}" class. This parent class has a prefiltering part and its task is to create a mapping between the \textbf{JobDescription} and the \textbf{ExecutionTarget}. The mapping is working but it will be change at the near future because the \textbf{JobDescription} class will be  recreate with \textbf{GIN Profile JSDL}. The mapping result is the \textbf{PossibleTargets} vector which content the "good" \textbf{ExecutionTargets}. The descendant broker classes task is to sort the \textbf{PossibleTargets} vector with its name type, for example: \textbf{RandomBroker} sort type is \textbf{Random}. The broker separated into a module therefore it is loadable.
