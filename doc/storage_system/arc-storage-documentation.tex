...
\note[JKN]{TODO: This paragraph is very unclear. All the 'could's makes it look like the TrustManager is just something planed in the far future. Are the attributes mandatory or optional?}The TrustManager section itself could have some attributes: \textbf{FromFile} could have a filename, if we want to get the whole TrustManager configuration from a separate file; and \textbf{Force} indicates (with `yes' or `no') that we want to ensure that our service does not even get the request if the client of the incoming connection is not trusted. 
...
\begin{description}
    ...
    \item[makeMountpoint(makeMountpointRequestList)] creates a mountpoint to an external storage element.

    \emph{makeMountpoint} is quite similar to the \emph{makeCollection}. It is used to provide access to an external storage system within the global namespace. \emph{makeMountpointRequestList} is a list of (\emph{requestID}, \emph{LN}, \emph{URL}, \emph{metadata}) pairs where \emph{metadata} is a list of (\emph{section}, \emph{property}, \emph{value}) tuples. \emph{section} contains the initial content of the catalog in the form of name-GUID pairs (these entries are hard links to the given GUID's with the given name). \emph{LN} is the requested logical name and \emph{URL} is the path to the external storage element to be accessed. The \emph{policies} section contains access policies, and the metadata section contains any other \emph{metadata} in key-value pairs. 

    The \emph{status} in the response is one of `\textbf{done}', `\textbf{mountpoint exists}', `\textbf{parent does not exist}', `\textbf{internal error}', `\textbf{failed}: credentials not found' or `\textbf{cannot create anything in mountpoint}'.

    \item[unmakeMountPoint(unmakeMountpointRequestList)] deletes mountpoints.

    \emph{unmakeMountpointRequestList} is a list of (requestID, LN) pairs with the logical names of the mountpoints. \emph{status} is one of `\textbf{removed}', `\textbf{no such LN}', `\textbf{Failed:} Credential not available'. 

    \item[CredentialDelegation(CredentialDelegationRequest)] delegates credentials.
\note[JKN]{TODO: This item has new style. Rewrite to make it similar to all the previous items. Also, second possible response looks very strange!}    
...
\note[JKN]{TODO: This item has new style. Rewrite to make it similar to all the previous items. Also, the responses look very strange!}    
...
The Bartender utilizes gateway modules to access file listings and transfer URL's from third-party storage elements. It works as an optional backend component of the Bartender service. The \emph{Gateway} module is based on four basic methods; \emph{get}, \emph{put}, \emph{list} and \emph{remove}. Since the task is to contact external storage element, which requires the user to delegate the credentials so that it can communicate on behelf of the user\note[JKN]{TODO: What does this sentence mean? Please rewrite.}.     

In order to access the files and directories from the external storage element, limited user credentials are passed to the \emph{Bartender} service. Users can also remove their \emph{credentials} from the service side. These credentials are used in the methods \emph{get}, \emph{put}, \emph{remove} and \emph{list} of the \emph{Gateway} module. These limited credentials are not used in any type of transfer. While listing files and directories, the Gateway uses the user credential, whereas in the \emph{get} and \emph{put} methods, it only uses the credentials to check the validity of URL's before sending them to the user.
% subsection implementation (end)
...
\note[JKN]{TODO: There is nothing about status or plans. Either add some status and plans or change subtitle of document.}
