...
\emph{A-H}: the \textbf{A-Hash}, which is a currently centralized, later distributed database;
\emph{L}: the \textbf{Librarian}, which stores the metadata and hierarchy of collections and files, the location of replicas, and health data of the Shepherd services, using the A-Hash as database;
\emph{B}: the \textbf{Bartender}, which provides a high-level interface for the users and for other services;
\emph{S}: the \textbf{Shepherd}, which provides a simple interface for storing files on storage nodes.
} }
\end{figure}
...
\section{Other plans} % (fold)

\subsection{Integrating third-party storage solutions} % (fold)
...
% subsection integrating_third_party_storage_solutions (end)
