...
proof-of-concept test results, deploying the storage system
distributed between three different countries. 
~\\~\\~
\end{abstract}
%-----------------------------------------------------------
...
possibility to introduce modifications in the future. %% The HED service container gives
...
elements in the order of several hundreds of terabytes in a single
...
Bigtable. Chubby has several features similar to the distributed A-Hash service,
...
offered by our Gateway service. However, being built as a middleware on top of
...
Daemon (HED). There are four kinds of pluggable components having well
...
atomic unit. The scope of the ARC data management system in the foreseen
...
external store. %% For the experiments we use dCache with gridftp
...
\item The \textbf{inter-service authorization} takes care of the
  ...
  Shepherds communicate with the Librarians, the Librarians talk with
...
\item In total we are running five Shepherds: Three at USIT, having
...
The following test where considered:

\begin{itemize}
\item Test 1: The distribution after uploading 10 large size (1 GB)
  files with one replica each.  
\item Test 2: The distribution after uploading 100 small size (1 kB)
  files with one replica each, and with simultanious uploading
\item Test 3: Distribution after repeating Tests 1 and 2 with three replicas.  
...
Figure \ref{fig:FileDistribution} illustrate two different scenarios,
...
uploading, but with the clients requesting three replicas each. The
...
%% &\small{UPPMAX}&\small{USIT-1}&\small{USIT-2}&\small{USIT-3}&\small{NIIF}\\
%% \hline
%%            \small{Before(MB)}&4888&7820&6843&7820&3320\\
%%            \small{During(MB)}&8798&9776& - &9775&2344 \\
%%            \small{After(MB)}&8798&6843&4888&8798&1367\\
%% \hline
%% \end{tabular}
...
Shepherds when one of them is disrupted. Interestingly, the storage load
automatically evens out when the Shepherd comes back online. When the
...
obsolete replicas. Interestingly, the NIIF node actually got fewer
...
%% International Conference on Computing in High Energy and Nuclear Physics (CHEPê07) IOP Publishing 
