...
\begin{center}
\textsc{\large The ARC Computational Job Management Component - A-REX}
...
The aim of the A-REX is to take care of job pre- and post-processing.
...
The A-REX implements Web Service (WS) interface which provides a way
to submit and control computational tasks (jobs) to be executed by
the A-REX and undelying Local Resource Management System.
...
instalaltion and configuration of whole system refer to other documents
avaialble at \url{http://www.nordugrid.org/papers.html}. }


\section{Main concepts\label{sec:main concepts}}

A job is a set of input files (which may include executables), a main
executable and a set of output files. The process of gathering input
files, executing a job, and transferring/storing output files is called
a \emph{session}.
...
is accessible by the user from outside through HTTP(S) protocol. Any
file created outside the SD is not controlled by the A-REX. Any exchange
of data between client and A-REX (including also program modules)
is done through HTTP(S) protocol. A URL for accessing input/output
files is  obtained through the WS Local Information Description Interface
(LIDI) of A-REX.

Each job gets an identifier (\textit{jobid}). This is a handle - WS-Addressing
\cite{jsdl} XML document - which identifies the job in the A-REX
and the Information Interface.

Jobs are initiated and controlled through WS interface. Complete job
descriptions (JD)are passed to the A-REX through WS in JSDL\cite{jsdl}
coded description. Input data files and job executables are transferred
separately through the same interface, as described in the next section.
...
in the SD or associated cache area. There are two ways to put a file
into the SD:
...
during any stage of processing, no attept is made to transfer those
...
executable is not 0) all files from furst group are transfered to
second one.
...
\textbf{PENDING} to the current state name in the job's status desription.
...
\emph{Local Resource Management System} (LRMS). The correspoding backends
for many LRMSs are provided with the default installation. If the
local job submission is successful the job moves to the \textbf{Executing}
state. Otherwise it moves to \textbf{Finishing}. It is possible to
limit the agregate number of jobs in \textbf{Submiting} and \textbf{Executing}
states.
...
obtained by querying the Informatrion Interface of the A-REX. If a
...
of current state of the job. Possible values and corrsponding external
job states are :

\begin{itemize}
\item ACCEPTED/\textbf{Accepted}
\item PREPARING/\textbf{Preparing}
\item SUBMITTING/\textbf{Submiting}
\item INLRMS/\textbf{Executing}
\item FINISHING/\textbf{Finishing}
\item FINISHED/\textbf{Finished}
\item CANCELING/\textbf{Killing}
\item DELETED/\textbf{Deleted}
\end{itemize}
\end{itemize}
...
default logger service configured in the configuration (This feature
...
\textit{url} - ordinary URL for gsiftp, ftp, http, https protocols
with the addition of '\textbf{replica location service url}' (RLS
URL).\\
...
the BES service. The A-REX does not implement remote control of service
functionality. Hence the BES-Management port-type is not functional.
...
\emph{ChangeActivityStatus} (see Appendix D). This operation provides
...
\ref{tab:Job-states-definitions}. Their meaning is described in section
\ref{Section: Job Flow}.
...
and i snot implemented yet.\tabularnewline
\hline
\end{tabular}
\end{table}
...
The A-REX also supportd the Delegation Interface (see Appendix E).
...
implements two operations: initiatialization of credentials delegation
...
\item [{\emph{nordugrid}}] - desription of computing resource that uses
NorudGrid LDAP schema {[}] converted to XML document.
\item [{\emph{Domains}}] \emph{-} description of a computation resource
...
\item [{\emph{Environment}\ (POSIX)}] - environement variable.
\end{lyxlist}
...
The A-REX can cache input files. Caching is enabled if corresponding
command is present in configuration file. The A-REX does not cache
files marked as executable in job. Caching can also be explicitly
turned off by user for each file by using \textit{cache=no} option
in URL (for URL options read {}``Protocols, Uniform Resource Locators
(URL) and extensions supported in ARC'' \cite{urls}). The disc space
occupied by cache is controlled by removing unused files. For more
information look in section \ref{SubSection: ConfigFile}.

\emph{NOTE: The caching algorithm used by A-REX and described above
is currently being replaced by a better one which is no robust and
...
If a job requests an input file which is subject for cacheing, it
is stored in the cache directory instead of the SD. The file is made
available to the job by either soft-linking it in the SD or copying
it to the SD. The latter option is more secure and hence advised.

Before downloading a file the A-REX tries to determine it's size and
then tries to preallocate space in the cache directory by writing
an empty file of the same size. If this fails (possibly because the
file system has no more space), it tries to the remove oldest cached
files which are not being used by any job. This means that \textbf{the
real hard limit of cache size is the space available in the file-system}.
...
Before giving access to an already cached file, the A-REX contacts
the original file source to check if the user has suficient access
rights to the file. Not all protocols make this check possible. Also,
file creation or validity times are checked to make sure the cached
file is fresh. If it is impossible to obtain creation and invalidation
times, the file is invalidated 24 hours after it was downloaded.

The A-REX checks the cache periodically. If the space used by cache
exceeds the high water-mark given in the configuration file (\emph{softsize}),
it tries to remove the oldest unused files until cache size drops
below the low water-mark. This sets the soft limit of cache size.
...
the Unix user who started A-REX and are readable by everyone.
...
files to the specified SEs and registration at an Indexding Service
...
Service. It reads and modifies cache informational files \emph{old}
and \emph{new} (as described in section \ref{sec:cache}). Configuration
is read directly from the GM's configuration file (see section \ref{SubSection: ConfigFile}).
...
\item \emph{cluster+qju.pl} - Collects and generates information about computing
resource as XML document in NoruGrid and Glue 2 format.
\end{itemize}
...
\$ARC\_LOCATION/bin -- tools

\$ARC\_LOCATION/libexec -- program modules used by A-REX

/etc -- central configuration file - location used by default

\$ARC\_LOCATION/lib/arc -- service module

\end{LyXParagraphLeftIndent}
The A-REX also uses following directories:

\begin{itemize}
\item \textit{session root directory} - Tthis is the directory in which
a user's SDs are created. It's location is configurable per UNIX user.
...
root directory} resides on a filesystem which does not limit the capabilities
of the \emph{root} user (as does for example NFS with \emph{root\_squash}
option).\\
...
local identity used to run a job. Some filesystems require users to
have \emph{execute} permission on the \textit{session root directory}
...
must be used to transfer files to execution nodes. For more information
see section \ref{sub:LRMS}.
...
\subsection{\label{SubSection: ConfigFile}Configuration of the A-REX}

Due to historical resons configuration of the A-REX is split into
2 parts. The small part is located inside HED configuration (see appendix
B) for schema and description of supported elements. It refers to
...
requires installation of A-REX and all realted software to be accessible
...
The default is \emph{no}. Only files downloaded through \emph{meta-url}
are registred. Registration is done to same service used for obtaining
information about file. For this opeartion credentials of the A-REX
(host key and certificate) are used. If required new files storage
location is registered at Indexing Service with quasi-url \emph{cache://hostname/}
...
a user to which Grid identity is mapped in order to access filesystem
...
Currently A-REX does not implement any internal authorization tecniques.


\subsection{\label{sub:LRMS}LRMS support}

The GM comes with support for several LRMS. And this number is slowly
growing. Features explained below are for \textbf{PBS/Torque} backend.
This support is provided through \textit{submit-pbs-job}, \textit{cancel-pbs-job},
...
input files have been gathered in the session directory and show the
actions taken right after the job's script starts. Pictures b) shows
the situation while the job is running and the actions which are taken
right after it has finished. Pictures c) illustrates the final situation,
when the job's output files are ready to be uploaded to an external
storage element or be downloaded by the user.
...
The A-REX can run specially prepared \emph{BASH} scripts prior creation
...
shell variables. With argument '0' scripts are run by the A-REX on
...
For publicly available runtime environments please see RTE \url{http://gridrer.csc.fi/}
.


\section{Installation\label{sec:installation}}

The A-REX is installed as part of ARC 1 middleware and is available
in NorduGrid download area at ARC1 Download \url{http://download.nordugrid.org/software/nordugrid-arc1/}
.There are packages available for various Linux distributions. The
A-REX comes in nordugrid-arc1-arex package. Source code ready for
...
accounts to run these services at the cost of some functionality loss
as described below.

There are no drawbacks of running GFS with \emph{gaclplugin} or \emph{fileplugin}
under a \emph{non-root} account as long as the only Grid identity
used is that of the user who runs the services and all served files
and directories are owned by the server's account. 

In order for GM and GFS with \emph{jobplugin} to be able to cooperate,
both services must be run either by the same non-root account or one
of the services must be run under the \emph{root} account. This is
needed because the two services communicate over a local filesystem,
hence they must have \emph{full} access to the same set of files.

As long as GFS with \emph{jobplugin} is run under a non-root account,
no mapping from Grid identity to local UNIX account takes place. All
alowed Grid users are assigned the server's account and are then processes
by GM using same account. The only way to overcome this limitation
is to run one GFS per local account with proper access control configured.

Because GM has to impersonate a user's local account while communicating
with the LRMS, it can serve only the account it is run under (unless
it is run under the \emph{root} account, of course). As in the case
of GFS, multiple instances of GM may be run, one per local account.
This solution has some drawbacks. The GM looses possibility to share
cached files among serviced users. In addition, it is not possible
to control the load on a frontend by limiting the number of simultatenuosly
running \emph{downloader} and \emph{uploader} modules.

One also has to take into account that the private part of the GSI
infrastructure (private key of a host at least) has to be duplicated
for every account used to run GFS.


\section*{Appendix A. Session directory access through HTTP(S) interface}
...
to him/her. Directory names are job identifiers, each representing
one job. These directories are directly connected to session directories
of jobs and contain the files and subdirectories that are visible
on the frontend.

If a job's xRSL has \emph{gmlog} specified, then the job's directory
also contains a virtual subdirectory of that name holding files with
information about the job as created by GM. The 'errors' and 'status'
files are especially usefull for troubleshooting. 'errors' contains
the stderr output of the various modules run by GM during job processing
...
named \char`\"{}new\char`\"{} -- used to submit new jobs -- and another
directory named ''info''. The latter has subdirectories named after
job ids, each of which contains files with information about a job.
These are the same files that can be accessed in the subdirectory
specified through \emph{gmlog}.


\subsection*{Submission}

Each xRSL put into the {}``new'' directory by a gridftp client is
treated as a new job's description. GFS's jobplugin parses the xRSL
and returns the client a positive response if there were no errors
in the request.
...
should be transferred from the client's machine, the client must upload
them to that directory under specified names.

The job identifier reserved by GFS to a new job must be somehow communicated
back to the client. Within the bounds of the FTP protocol, this is
achieved in the following way. Prior to uploading the xRSL, the client
issues a CWD command to change the current directory to {}``new''.
In turn, the server responds with a redirect to the new session directory
named after the reserved identifier. The client has now found out
the job's id, and proceeds with uploading the xRLS.
...
xRSL may consist of only 2 parameters - \emph{action} for action to
be performed, and \emph{jobid} to identify the job to be affected.
...
\section*{Appendix C. Configuration schema of A-REX}
...
~~~~~~<xsd:element~name=\char`\"{}defaultLocalName\char`\"{}~type=\char`\"{}xsd:string\char`\"{}~minOccurs=\char`\"{}0\char`\"{}/>
...
\section*{Appendix C. Error messages of A-REX}
