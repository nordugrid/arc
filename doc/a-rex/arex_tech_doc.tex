...
\emph{ChangeActivityStatus} (see Appendix C). This operation provides
...
The A-REX also supportd the Delegation Interface (see Appendix D).
...
is read directly from the A-REX's configuration file (see section
\ref{SubSection: ConfigFile}). It is run by the A-REX every 5 minutes.
...
as seen on \textbf{frontend} machine. Corresponds to \emph{shared\_scratch}
configuration command.
...
way. Corresponds to \emph{shared\_filesystem} configuration command.
...
performed. Pictures a) correspond to the situation right after all
input files have been gathered in the session directory and show the
actions taken right after the job's script starts. Pictures b) show
the situation while the job is running and the actions which are taken
right after it has finished. Pictures c) illustrate the final situation,
...
The A-REX service is a plugable module of the HED. Si first You need
to setup your HED - see HED documentation. 

Then You need to add A-REX to the HED configuration. Add new <Name>
element inside <Plugins> containing string \emph{arex}. That will
make HED load libarex plugin library. Then add new <Service> element
with attribute name=''a-rex''. That will instantiate A-REX service.
Now to make service accessible extend <Plexer> element with new <next>
refering to service's id. Write <Service> element carefully - here
is an example:
...
Please read \emph{User Manual {[}]} for usage instructions.
...
of application dependant files in SD.

The BES defines job identifier as WS Addressing {[}] Endpoint Reference
(EPR) - XML document. The EPR is extendable and the A-REX adds it
own element JobSessionDir belonging to namespace http://www.nordugrid.org/schemas/a-rex
as direct child of ReferenceParameters element. This new element contains
the URL of SD.

Obtained URL shoud be extended with file names relative to SD and
...
supported shich returns HTML encoded non-recursive list of files and
firectories. The files and subdirectories have their URLs inside HTML
element <A>.


\section*{Appendix B. Configuration schema of A-REX}
...
\section*{Appendix D. Delegation WSDL}
...
\section*{Appendix E. ARC extensions for JSDL schema}
...
\section*{Appendix F. Error messages of A-REX}
