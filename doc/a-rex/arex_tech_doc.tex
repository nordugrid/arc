...
of so called \emph{Computing Element} (CE). 

It takes care of job pre- and post-processing. This comprises the
stage-in of files containing input data or program modules from wide
range of sources and the transfer or storing of output results.

The A-REX implements a Web Service (WS) interface which provides a
way to submit and control computational tasks (jobs) to be executed
by the A-REX and he underlying Local Resource Management System.
...
\cite{ws-addr-soap} XML document - which identifies the job in the
A-REX and the Information Interface.
...
Finished&
Finished&
Job finished successfully\tabularnewline
\hline 
Finished&
Deleted&
Job finished and was left in A-REX too long\tabularnewline
\hline 
All&
Pending&
...
\item [{\emph{IndividualCPUCount}}] - same as 6.
\item [{\emph{Environment}\ (POSIX)}] - environment variable.
\end{lyxlist}
...
If a job requests an input file which is subject for caching, it is
stored in the cache directory instead of the SD. The file is made
...
the original file source to check if the user has sufficient access
...
Currently supported LRMS are PBS/Torque, Condor and SGE. Also \emph{fork}
...
are registered. Registration is done to same service used for obtaining
information about file. For this operation credentials of the A-REX
...
on file system. If \textit{low\_mark} is omitted it becomes equal
to \textit{high\_mark}. By default this feature is turned off. To
turn it off explicitly \textit{cachesize} without parameters should
be specified. If turned off cache will grow up till it fills whole
file system.
...
like NFS. Corresponds to an environment variable RUNTIME\_NODE\_SEES\_FRONTEND
(see section \ref{sub:LRMS}).
...
\item joboption\_args - command to be executed as specified in the JD.
...
The A-REX service is a plugable module of the HED. So first You need
...
Then add new <Service> element with attribute name=''a-rex''. That
will instantiate A-REX service. Now to make service accessible extend
<Plexer> element with new <next> referring to an id of the service.
...
the A-REX adds it own element JobSessionDir belonging to namespace
http://www.nordugrid.org/schemas/a-rex as direct child of ReferenceParameters
