...
\begin{tabular}{|c|c|c|>{\raggedright}p{10cm}|}
\hline 
...
by the A-REX yet. \emph{Accepted} is first reported state.\tabularnewline
...
\cline{2-2} \cline{3-3} \cline{4-4} 
&
\multicolumn{1}{c|}{Queued}&
\multicolumn{1}{c|}{INLRMS}&
\multicolumn{1}{p{10cm}|}{Job entered local batch system but is not runnning now. This state
is not recognised by the A-REX yet. \emph{Executing} is reported instead }\tabularnewline
\cline{2-2} \cline{3-3} \cline{4-4} 
\multicolumn{1}{|c|}{}&
Executing&
INLRMS&
...
is reported only if job is \emph{Pending}.\tabularnewline
\cline{2-2} \cline{3-3} \cline{4-4} 
\multicolumn{1}{|c|}{}&
Killing&
CANCELING&
...
not remember this request. \emph{Failed} is reported instead.\tabularnewline
\hline 
\multicolumn{1}{|c|}{Failed}&
\multicolumn{1}{c|}{Failed}&
FINISHED&
\multicolumn{1}{l|}{There was a failure during execution}\tabularnewline
\hline 
Finished&
Finished&
FINISHED&
...
and is not implemented yet.\tabularnewline
\hline
\end{tabular}
\end{table}
...
in case of big data storage available only on NFS server which does
...
\end{itemize}
For example \emph{joboption\_arg\_\#} could be changed to wrap main
...
\section{Session directory access through HTTP(S) interface}
...
\section{Configuration schema of A-REX}
...
\section{ARC extensions for JSDL schema}
...
\section{Error messages of A-REX}
