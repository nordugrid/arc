...
to know whether they are in use by a currently running
job. \textbf{IMPORTANT:} If a cache is mounted from an NFS server and
the GM is run by the root user, the server must have the
\emph{no\_root\_squash} option set for the GM host in the
\emph{/etc/exports} file, otherwise the GM will not be able to create
the required directories. Note that when running A-REX under a
...
\item \textbf{\textit{remotecachedir}}\textit{=path {[}link\_path]} -
  ...
  a file is not available in paths specified by cachedir, the GM looks
