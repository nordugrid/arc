...
The A-REX can cache input files. Caching is enabled if one or more
cache directories are specified in the configuration file. Caching can
also be explicitly turned off by the user for each file by using
the\textit{ cache=no} option in the URL (for URL options read
{}``Protocols, Uniform Resource Locators (URL) and extensions
supported in ARC'' \cite{urls}). The disk space occupied by the cache
is controlled by removing old files.  For more information on
configuration look in section \ref{SubSection:ConfigFile}.
...
a directory to store cached data (see \ref{sec:cache}). Multiple
