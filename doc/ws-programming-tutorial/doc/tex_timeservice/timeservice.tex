...
The proper class definition of the object \textit{TimeService} is defined in the corresponding header file which is not shown here. \task{show the header file, or at least the important bits of it - sm}
...
configuration and the message chain of the service. The time service uses the namespace \textit{ArcService}.
...
The method \textit{process} is required by the inherited class \textit{Service}.
It is defined in line~\ref{lst_code:time_cpp_process} and realises the desired service,
here: the calling of functions provided by system libraries to determine the current time. 
...
Within the method \textit{process}, the service first determines the current time and then creates a SOAP payload using the object \textit{PayloadSOAP} on line~\ref{lst_code:time_cpp_process_payloadSOAP}. 
The object \textit{PayloadSOAP} provides enough functionality to create an XML message that is conform to the SOAP protocol.
...
When nothing unexpected happened while the message is processed, the function returns an \textit{MCCStatus} set to \textit{OK} as seen in line~\ref{lst_code:time_cpp_return}.
...
Up to now we have implemented a server program, the code that runs on a particular machine and waits for other programs that ask it for the time. What is still needed is the client. If the server spoke the http protocol, then we could use our web browser. But it does not - wait, stop - in a way it does, but the server's SOAP envelope renders the reply rather awkward to read. \task{someone please check - SM; I think you are wrong (I like to read SOAP messages) :-) - MG}  The server exists only once. But there may be many many clients requesting it in parallel - on the same machine or across the internet. The client may be directly controlled by a human (like the web browser) or be some cron job or other software initiating the request.
...
Actually, if available, in many cases the client may use the configuration file of the server. \task{seen for the arcecho, but is that true? sm}
