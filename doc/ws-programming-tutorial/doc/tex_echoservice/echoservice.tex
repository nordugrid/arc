...
The next service presented is the Echo Web Service.
...
The WSDL service specification enables a foreign programmer to implement a suitable client which interfaces the service in a proper way.
...
A SOAP fault message is always located within the body element and can appear only one time. 
It indicates error messages in case the port type \textit{solicit-response} (input, output, fault) has been chosen.
An element called \textit{fault} encapsulates diffrent types of elements which are describing the fault.
...
For morde information about WSDL it is recommended to check the internet address~\href{http://www.w3.org/TR/wsdl}{http://www.w3.org/TR/wsdl}.
...
Regarding to the source code of the Echo Web Service basically three changes of the Time Web Service source code are required.
...
The creation of the SOAP client is unchanged and the request is created in the lines subsequent to line~\ref{lst_code:echo_client_cpp_request}.
It is processed in the code after line~\ref{lst_code:echo_client_cpp_response} in which is also the response is received.
