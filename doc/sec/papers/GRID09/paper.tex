...
We consider two situations in terms of the authenticating between user agent (client) and IdP: 
...
subsequent authentication, in a short period (we setup this period as 30 minutes).

Considering the success of authentication from IdP can be regarded as valid for the whole
session between client and service, we also consider two situation in terms of the 
using of authentication result between client and service: session re-used, and session non-reused.
...
the session as longer as possbile is also a way to improve the performance of 
Web Service invokation with SAML2SSO profile integrated as authentication approach.

Figure \ref{fig:SAML2SSO_thread10_nosso_and_sso} depicts the performance comparison between singled 
sign-on disabled and singled sign-on enabled, for 10 clients running simultaneously 
against the ``echoString'' service enhanced with SAML2SSO profile, with session not re-used.

The average response time of ``SSO enabled'' is about 1700ms, while the throughput is about
340 requests per minutes. The average response time of ``SSO disabled'' is about 3000ms, while 
the through put is about 215 requests per minutes. The comparison shows that the performance 
...
this set of experiments, the session is not reused, and single sign-on enabled and singled sign-on
disabled are compared. 

For the situation of singled sign-on disabled, the average response time 
increases linearly with the client number, while the throughput increases initially and then 
keeps in a fixed value (around 215 requests per minute) when the client number is bigger 
than 4. When the client number is four, the response time is around one second, while the 
throughput of the service is 215 requests per minutes which is almost the fixed value.

For the situation of singled sign-on enabled, the average response time 
also increases linearly with the client number, while the throughput also increases initially and then 
keeps in a fixed value (around 340 requests per minute) when the client number is bigger 
than 5. When the client number is five, the response time is around 880ms, while the 
throughput of the service is 340 requests per minutes which is almost the fixed value.

We speculate that in order to get acceptable response time, for the case of single sign-on disabled, 
there should be less than four (including four) concurrent clients if they need to continuously 
participate one SAML2SSO profile; for the case of single sign-on enabled, 
...
can claim that for the case of of single sign-on disabled, around 215 clients randomly can be 
...
Considering the time interval of the usage of SLCS client should be similar to the lifetime of SLCS
credential which is 12 hours by default, and the lifetime of successful authentication session is 
30 minutes by default, enabling single sign-on is not useful in real deployment because the successful 
authentication session will be expired before the next time the SLCS client is used, therefore, 
