...
Grid authentication by using \texttt{SAML2SSO} profile; Section~\ref{sec:fedidtoX509} describes
...
Figure~\ref{fig:SAML2SSOUML} are similar to those steps depicted in the Web Browser SSO profile, with
...
the username and password again. So the user (or the client on behalf of that user) can travel across multiple
...
Many widely used Grid middlewares are based on GSI which requires mutual X.509
...
of Figure~\ref{fig:SAML2SSOUML}) including a signed X.509 certificate and a CA certificate that is used 
...
well as the performance related to the SAML2SSO profile, short-lived credential
...
demonstrate the performance HED.
...
as it gets from input. For the client side, we developed a client by using the client API of ARC, to
...
Throughput is computed by counting number of invocations per second.
...
Figure \ref{fig:SAML2SSO_thread10} depicts the average response time and
