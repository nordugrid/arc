...
hash of hashes with the sctructure described in table \ref{t:lrms_info}.
...
\vspace*{3mm}
The following properties referring to job counts are obtained from GridManager's status files:

\begin{tabular}{|l|p{12cm}|}
\hline
Property & Meaning \\
\hline \hline
TotalJobs & Number of grid jobs which are not yet finished \\
\hline
RunningJobs & Number of grid jobs running \\
\hline
WaitingJobs & Number of grid jobs queued in the LRMS and waiting to be submitted to the LRMS \\
\hline
\end{tabular}
...
\hline
IssuerCA & from the \emph{hostcert.pem} file \\
\hline
TrustedCA & from scanning the \emph{certificates} directory \\
\hline
...
\hline
MappingQueue & from the configuration \\
\hline
\end{tabular}

\vspace*{3mm}
The following properties are taken from the corresponding values returned by
the LRMS plugin. Units are converted as necessary.

\begin{tabular}{|l|p{12cm}|}
\hline
Property & LRMS plugin property \\
\hline \hline
...
Property & Value\\
\hline \hline
MaxTotalJobs & maxjobs from configuration\\
MaxRunningJobs & \\
MaxWaitingJobs & \\
\hline
\end{tabular}
