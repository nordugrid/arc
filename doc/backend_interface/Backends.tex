...
\begin{itemize}
\item \texttt{cluster+qju.pl} - driver for information collection. It calls all other information collectors and prints results in XML. The information collection is done by one single invocation of this script
\item \texttt{InfoCollector.pm} - base class for all information collectors (i.e.: all files \texttt{*Info.pm}).
\item \texttt{InfoChecker.pm} - used by InfoCollector to validate options and results against a sort of ``schema'', as implemented by \texttt{InfoChecker.pm}.
...
\item \texttt{LRMSInfo.pm} - collects information that is LRMS specific (queues, jobs, local user limits ...) by calling the appropriate LRMS plugin. It also does validation of input options to the plugin and of the data returned by the plugin. 
...
\item \texttt{ARC0ClusterSchema.pm} - ``schema'' of the info produced by ARC0ClusterInfo
\item \texttt{ARC1ClusterSchema.pm} - ``schema'' of the info produced by ARC1ClusterInfo
\end{itemize}
...
used to validate input options by \texttt{LRMSInfo.pm}.


\subsubsection{lrms\_info}

\texttt{lrms\_info} returns all LRMS specific information. This
includes information about the cluster, queues, jobs and local users. The
function is called with an options hash of hashes. It should then return an
hash of hashes with the following keys and the appropriate value.

\begin{table}
\caption{The \texttt{lrms\_info} hash} \label{t:lrms_info}
\begin{center}
\begin{tabular}{|c|c|}
\hline
key & value\\
...
\hline
maxwalltime & limit on maximum wall time for a job in this queue in minutes\\
\hline
...
key & value\\
\hline \hline
status & LRMS jobstatus mapping\footnotemark[1]:
Running->'R', Queued->'Q', Suspended->'S', Exiting->'E', Other->'O'\\
\hline
...
\hline
\end{tabular}
\footnotetext[1]{LRMS job states mapping is described in\cite{is}}
\end{center}
\end{table}
