...
\begin{enumerate}
\item to allow the GridManager (GM), which is resided in A-REX, to manipulate
...
anything else needed. Then it submits the job to the local LRMS.
This is normally done by generating a native job script for the LRMS
and then running the local submit command, but it can also be done
...
in the ARC jobstatus directory. Then it will call the gm-kick program
to notify GM about the finished job. Subsequently, the GM starts finalizing
the job.

Generally, two approaches are taken to find jobs that are done. One
is to ask the LRMS. First all started grid jobs are found in the jobstatus
directory%
...
system maintained by Sun. It runs on Linux, and Solaris.
